...
const MSG = {
  ...
  css: `
    ...
    /* Code wears a wash of the chrome's own yellow - the colour that already
       means "this is walkdown's, look here" on the badges - so a path or an
       id stands apart from the prose around it in more than typeface. Tinted
       from the theme token, so a delivery with no theme still gets a wash. */
    .wd-text code { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: .92em;
      padding: .05em .3em; border-radius: .25rem;
      color: color-mix(in oklch, var(--color-warning, oklch(84% 0.15 85)) 88%, currentColor);
      background: color-mix(in oklch, var(--color-warning, oklch(84% 0.15 85)) 13%, transparent); }
    .wd-text pre { padding: .4rem .5rem; border-radius: .3rem; overflow-x: auto; white-space: pre;
      background: color-mix(in oklch, var(--color-warning, oklch(84% 0.15 85)) 9%, transparent);
      border: 1px solid color-mix(in oklch, var(--color-warning, oklch(84% 0.15 85)) 22%, transparent); }
    ...
    .wd-text .wd-ref.font-mono { color: color-mix(in oklch, var(--color-warning, oklch(84% 0.15 85)) 88%, currentColor); }
