...
const MSG = {
  ...
  messages(thread) {
    return [
      ...
      { author: thread?.author, via: thread?.via, created: thread?.created, body: thread?.body, added: thread?.added, opens: thread?.kind ?? 'note', options: thread?.options, chosen: thread?.chosen },
      ...(thread?.replies ?? []),
    ].filter((m) => m && (m.body ?? '') !== '');
  },
