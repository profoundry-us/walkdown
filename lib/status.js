...
export function deriveStatus(blueprint, { target } = {}) {
  ...
  //        awaits folding into the rule).
  const attention = [];
  for (const row of rows) {
    // Sign-off states still owe a judgment: approving a spec is not judging
    // the build, so the rule stays in the human queue until one lands.
    if (row.verify.includes('human') &&
