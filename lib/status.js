...
export function deriveStatus(blueprint, { target, checkRefs } = {}) {
  ...
  /*
   * The verify queue is a person's own words waiting on their own look (ADR
   * 0005 §6): feedback and requests, never a judge's finding - that closes
   * on the next signed pass of its rule - and never an agent's observation,
   * which the agent settles. Counted per RULE, because the gesture that
   * clears it is a verdict on the rule, and a queue of 185 thread ids was a
   * queue nobody worked. A thread with no rule stands on its own.
   */
  const verifyByRule = new Map();
  for (const { data: t } of threads) {
    if (!t?.id) continue;
    const rule = t.anchor?.rule ?? null;
    if (t.status === 'addressed') {
      if (!waitsOnPerson(t)) continue;
      ...
      /*
       * Under the rule only where a verdict on the rule is what clears it:
       * feedback, on a rule that can still be judged. A request is verified
       * from its own screen - a pass leaves it alone (n-0296) - and a note
       * on a retired rule has no rule to walk; grouped under the rule, each
       * kept it in the walk queue after every pass, with nothing there to
       * do (one-stream and no-framing, 2026-09-16).
       */
      if (!rule || !closesOnVerdict(t) || !rows.some((r) => r.rule === rule)) {
        attention.push({ who: 'human', action: 'verify', thread: t.id, rule, unjudged });
        continue;
      }
      const item = verifyByRule.get(rule) ?? { who: 'human', action: 'verify', rule, threads: [], unjudged: false };
      item.threads.push(t.id);
      item.unjudged ||= unjudged;
      verifyByRule.set(rule, item);
  ...
  attention.push(...verifyByRule.values());
