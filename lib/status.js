...
export function deriveStatus(blueprint, { target, checkRefs } = {}) {
  ...
  const sweeps = sorted
    .map(({ data }) => data)
    .filter((r) => r?.kind === 'sweep' && r.created);
   ...
   */
  const rows = rules.filter(({ rule }) => !rule.retired).map(({ rule, story }) => {
    ...
    const agent = verify.includes('agent')
      ? toCell(agentCells.get(at), rule, undefined, 'agent', primary) : { state: 'na' };
    ...
    const acceptance = roles.map((role) => {
      ...
      const stale = ['pass', 'approved'].includes(e.status) && e.statementHash &&
        !hashMatches(e.statementHash, rule.statement);
      if (e.status === 'refining' || e.status === 'fail') return { role, state: 'sent-back', ...e };
