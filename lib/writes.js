 ...
 * machine it ran on - `via: agent` goes back to meaning only what n-0139
 * meant, a person's words that a machine typed. And WHETHER IT IS OPEN: a
 * decision is a record, filed closed as `recorded`, and enters no queue.
 */
export function openThread(blueprint, { kind, body, anchor, via = null, reason = null }) {
  /*
  ...
  if (kind === 'question' && reason) throw new Error('a question carries no reason; only a note does');
  const why = kind === 'question' ? null : (reason ?? defaultReason({ kind, via, author: who.username }));
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    const thread = {
      ...
      kind,
      ...(why ? { reason: why } : {}),
      author: machinesOwn ? 'agent' : asWho(who.username),
