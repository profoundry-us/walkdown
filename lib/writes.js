...
import { voiceFindings, voiceLines } from './voice.js';
import { acceptAttachments, writeAttachments } from './attachments.js';
import {
  defaultReason,
  HUMAN_ONLY,
  saysSomething,
 ...
 */
/*
 * The machine's own words pass the voice before they are filed (n-0323,
 * docs/13-voice.md): its body when it authors, and whatever it adds beside
 * a person's words. A person's words are theirs and are never gated. The
 * door refuses with the faults named, so the sentence is fixed rather than
 * filed; `asIs` files it anyway, said out loud on the command.
 */
function voiceGate(words, asIs) {
  if (asIs) return;
  const findings = [
    ...(words.author === 'agent' ? voiceFindings(words.body, { field: 'thread' }) : []),
    ...voiceFindings(words.added, { field: 'thread' }),
  ];
  if (findings.length)
    throw new Error(
      `the machine's words do not pass the voice (docs/13-voice.md):\n  - ${voiceLines(findings).join('\n  - ')}\nReword them, or file them as they are with --as-is.`,
    );
}
...
export function openThread(blueprint, { kind, body, anchor, via = null, reason = null, said: relayed = null, added = null, options = null, asIs = false, attachments = null }) {
  const choices = askOptions(kind, options);
  const pictures = acceptAttachments(attachments);
  /*
  ...
  /*
   * A retired rule is registered and then left alone (lib/schema.js). Two
   * questions were filed on one anyway, and each re-opened a decision its
   * retirement note had already recorded (q-0317, q-0335; Topher,
   * 2026-09-22). The refusal quotes that note, because it says where the
   * concern lives now. Replies and moves on threads already there go on
   * working: this is the door for NEW ones.
   */
  const gone = anchor?.rule ? collectRules(blueprint.features ?? []).find((r) => r.rule?.id === anchor.rule && r.rule.retired) : null;
  if (gone)
    throw new Error(`${anchor.rule} is retired - a new thread goes on the rule or screen that carries it now. Its note: ${String(gone.rule.retired).trim()}`);
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      // The pictures, once the id is ours: written beside the record and
      // named on it, so a thread and its pictures travel together.
      if (pictures.length) {
        thread.attachments = writeAttachments(dir, id, pictures);
        writeFileSync(join(dir, `${id}.yml`), stringify(thread));
      }
      return { id, thread };
    } catch (err) {
 ...
 */
export function reply(blueprint, id, { body, via = null, said = null, added = null, asIs = false, attachments = null }) {
  const who = whoIsActing(blueprint);
  const words = whoseWords(who, { via, said, body, added });
  voiceGate(words, asIs);
  const pictures = acceptAttachments(attachments);
  const shots = writeAttachments(blueprint.at.threads.path, id, pictures);
  return { thread: replyThread(blueprint, id, { ...words, attachments: shots }), by: who, as: words.author };
}
 ...
 */
export function transition(blueprint, id, { status, reason, via = null, chosen = null, attachments = null }) {
  const who = whoIsActing(blueprint);
  if (HUMAN_ONLY.includes(status)) mustBeDeclared(who, status);
  const actor = via ? 'agent' : who.username;
  // Pictures go with the reason, checked and written the way a reply's are.
  const pictures = reason?.trim() ? acceptAttachments(attachments) : [];
  const shots = writeAttachments(blueprint.at.threads.path, id, pictures);
  return {
    thread: moveThread(blueprint, id, { status, actor, reason, via: via === 'agent' ? null : via, chosen, attachments: shots }),
    by: who,
    as: actor,
  };
}
...
export function mutateThread(
  blueprint,
  id,
  { body = null, status = null, reason, via = null, said = null, added = null, asIs = false, attachments = null },
) {
  const who = whoIsActing(blueprint);
  ...
  if (words) voiceGate(words, asIs);
  const pictures = acceptAttachments(attachments);
  if (pictures.length && !words) throw new Error('a picture goes on a reply - say something with it');
  ...
  let thread = null;
  if (words) thread = replyThread(blueprint, id, { ...words, attachments: writeAttachments(blueprint.at.threads.path, id, pictures) });
...
/** Offer choices on a question already asked; validated the way a new question's are. */
export function offer(blueprint, id, options) {
  const choices = askOptions('question', options);
  if (!choices) throw new Error('say which choices to offer');
  return { thread: offerOptions(blueprint, id, choices), by: whoIsActing(blueprint) };
}
