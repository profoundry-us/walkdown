...
/*
 * The one author policy, and n-0121's open question is answered: REFUSE.
 *
 * A missing author used to fall back to the machine's username, on the
 * reasoning that the server cannot know who is at a browser that did not say.
 * True, and precisely why it must not guess - a login name filed as an author
 * is a record of who owns the laptop, dressed as a record of who said
 * something. Four separate defaulting bugs were found here, one per judging
 * pass, and every one of them was invisible because the default always
 * produced a plausible name.
 *
 * Refusing is loud, local, and fixable: the caller says who, or the write does
 * not happen. It stays ONE policy in ONE place so a new writer cannot
 * re-invent the handoff.
 */
const asWho = (author) => {
...
export function openThread(blueprint, { kind, body, anchor, author, via = null }) {
  /*
...
export const reply = (blueprint, id, { author, body, via = null }) =>
  replyThread(blueprint, id, { author: asWho(author), body, via });
 ...
 */
export const transition = (blueprint, id, { status, actor, reason, via = null }) =>
  moveThread(blueprint, id, { status, actor, reason, via });
