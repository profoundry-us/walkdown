...
async function withBrowser(fn, projectRoot) {
  ...
  clearTimeout(idle);
  // A browser that crashed or was killed stays an object that fails every
  // call; without this, one lost Chromium broke every picture until the
  // server restarted (setup dry run, 2026-09-23).
  if (browser && !browser.isConnected()) browser = null;
  if (!browser) {
    browser = await cr.launch();
    const mine = browser;
    mine.on?.('disconnected', () => {
      if (browser === mine) browser = null;
    });
  }
  try {
...
/** The browser pictures are taken in, if one is open. For tests that need to kill it the way a crash would. */
export const openBrowser = () => browser;
