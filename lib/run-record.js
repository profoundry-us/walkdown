...
export function writeRunRecord({
  ...
  if (!runsDir)
    throw new Error(
      `No runs directory for ${blueprintDir} — nothing declares it, so there is nowhere to file this run. \`walkdown where runs\` says what resolved; \`walkdown blueprint add <home>\` lists a blueprint.`,
    );
