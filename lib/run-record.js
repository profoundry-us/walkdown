...
import { isMachineName, ROLES } from './vocab.js';
...
/*
 * The signatures on a record, or null for "none stated".
 *
 * A signature is a role AND who accepted in it, because those are two facts
 * and one field was carrying both. A walk driven by one person while another
 * accepts beside them - the case that made this - has no honest shape while
 * the run knows only "which hats" and the run's actor supplies every name.
 *
 * Nothing else records the proxy. `actor` is who ran the walk, derived from
 * the machine and never nameable by a caller (n-0142, n-0143), so a signer
 * who is not the actor IS the record that somebody accepted through somebody
 * else. A second field saying so would be the same fact written twice, and
 * two copies of one fact eventually disagree (Topher, 2026-09-07, q-0225).
 *
 * A machine can never be a signer, in any spelling the actor gate refuses:
 * accepting is the one thing an agent may not do on somebody's behalf.
 */
export function normalizeSignatures(signatures, { actor = null } = {}) {
  if (signatures == null) return null;
  if (!Array.isArray(signatures)) throw new Error('signatures must be an array');
  const out = [];
  for (const raw of signatures) {
    if (!raw || typeof raw !== 'object')
      throw new Error('a signature is a role and the person who signed in it');
    const role = String(raw.role ?? '').trim();
    if (!role) continue;
    if (!ROLES.includes(role))
      throw new Error(`unknown role "${role}" (expected ${ROLES.join('|')})`);
    if (out.some((s) => s.role === role))
      throw new Error(`two ${role} signatures on one walkdown — a role signs once`);
    const signer = String(raw.signer ?? actor ?? '').trim();
    if (!signer) throw new Error(`the ${role} signature names nobody — a signature is somebody's`);
    if (isMachineName(signer))
      throw new Error(
        `"${signer}" cannot sign for ${role} — an agent may claim work, never accept it`,
      );
    out.push({ role, signer });
  }
...
export function aggregateResults(perTest, rulesById) {
  ...
  return [...byRule.values()].map((agg) => {
    ...
    if (rule?.statement && (agg.status === 'pass' || agg.status === 'fail'))
      result.statement_hash = formatHash(rule);
...
/*
 * The tree a verdict is ABOUT is the code, not the blueprint.
 *
 * These used to ask git about `blueprintDir`, which was the same directory
 * back when a spec lived at <project>/blueprint. On the layout `walkdown init`
 * now produces by default the home is in ~/.walkdown and is no repository at
 * all, so every record came out with no git_sha and no tree_hash - nothing for
 * a person digging into a verdict to look at (n-0190). Worse, a home inside a
 * git repository of its own, which is what versioning your dotfiles gives you,
 * stamped the HOME's sha onto a verdict about code sitting at another commit
 * somewhere else: provenance that misdirects rather than merely missing
 * (n-0192).
 *
 * `resolveLocations` has answered this all along - lib/locations.js resolves
 * `codeRoot` and says so in as many words - and nothing asked it. A caller
 * holding a loaded blueprint passes the answer it already has; everyone else
 * gets it from the same resolution that finds the runs directory.
 *
 * The spec hash still comes from the blueprint: that one really is about the
 * spec, which is the point of it.
 */
/** Short git sha of `cwd`, with a "-dirty" suffix for an unclean tree; null without git. */
/*
 * Where this tree's code can be read on the web, and at which commit - for a
 * link beside a check's source (n-0318). Best effort: GitHub's blob layout
 * only, null for anything else, and the full sha so the link outlives the
 * branch. A dirty tree links to the last commit, which is the most that can
 * be said.
 */
export function repoWeb(cwd) {
  try {
    ...
    const m = remote.match(/^(?:git@|ssh:\/\/git@|https?:\/\/)(github\.com)[:/](.+?)(?:\.git)?\/?$/);
    return m ? { web: `https://${m[1]}/${m[2]}`, sha } : null;
  } catch {
    return null;
  }
}
 ...
 *   codeRoot?: string | null, target: string, baseUrl?: string, actor: string,
 *   roles?: string[], signatures?: object[], kind?: string, perTest?: object[], rulesById?: Map<string, object>,
...
export function writeRunRecord({
  ...
  const signedBy = normalizeSignatures(signatures, { actor });
  /*
   ...
   */
  const at = given && givenCode ? null : resolveLocations({ spec: blueprintDir, cwd });
  const runsDir = given ?? at.runs.path;
  if (!runsDir)
    throw new Error(
      `No runs directory for ${blueprintDir} — nothing registered it, so there is nowhere to file this run. \`walkdown where runs\` says what resolved; \`walkdown import <home>\` registers a blueprint.`,
    );
  mkdirSync(runsDir, { recursive: true });
  const runId = nextRunId(runsDir, target, date);
  const code = givenCode ?? at?.codeRoot ?? null;
  const sha = code ? gitSha(code) : null;
  const tree = code ? treeHash(code) : null;
  const spec = specHash(blueprintDir);
  const record = {
    run_id: runId,
    created: date.toISOString().replace(/\.\d+Z$/, 'Z'),
    actor,
    ...(signedBy ? { signatures: signedBy } : signedAs ? { roles: signedAs } : {}),
    kind,
    target,
...
export function writeSweep({
  ...
  if (!runsDir)
    throw new Error(
      `No runs directory for ${blueprintDir} — nothing registered it, so there is nowhere to file this sweep.`,
    );
  mkdirSync(runsDir, { recursive: true });
  const runId = nextRunId(runsDir, target, date);
  const code = givenCode ?? at?.codeRoot ?? null;
