 ...
 */
import { RESULT_STATUSES, ROLES, statusesFor, THREAD_KINDS, THREAD_REASONS, TIERS } from './vocab.js';
...
export const RULE_SHAPE = [
  ...
  /*
   * Steps are not documentation. They are what the checks get built FROM: a
   * statement is one sentence and the given/when/then is where the edge cases
   * are written down, so a rule without them hands whoever implements it a
   * sentence and asks them to invent the rest. Eleven rules had drifted into
   * that state before anybody noticed (n-0246), all of them checks-only
   * derivation law, which is exactly where the detail matters most.
   *
   * Retired rules never reach here - the gate above halts on them - because
   * a rule we stopped meaning owes nothing but a resolvable id.
   */
  {
    kind: 'required',
    field: 'steps',
    ...
    category: 'schema',
    message:
      'rule has no steps — given/when/then is what a check gets built from, and a statement alone leaves it to be invented',
  },
  {
...
export const THREAD_SHAPE = [
  ...
  /*
   * A note's reason decides where it closes (ADR 0005), so a word nothing
   * recognises is a note nothing will close. Absent is allowed - a note from
   * before reasons reads as feedback, and a person looks - but a wrong word
   * is a file that lies.
   */
  {
    kind: 'scalar-vocab',
    field: 'reason',
    options: THREAD_REASONS,
    skipFalsy: true,
    level: 'warn',
    category: 'threads',
    message: 'reason "{value}" is not one walkdown knows (' + THREAD_REASONS.join('|') + ') — nothing will close this note',
  },
  {
    kind: 'require-when',
    when: { field: 'status', is: 'recorded' },
    field: 'reason',
    level: 'warn',
    category: 'threads',
    message: 'recorded is a decision\'s status, and this note gives no reason',
  },
  {
    kind: 'require-when',
...
export function applyShape(doc, shape, ctx, report) {
  ...
  for (const check of shape) {
    ...
    switch (check.kind) {
      ...
      case 'scalar-vocab': {
        if (check.skipFalsy && !doc[check.field]) break;
