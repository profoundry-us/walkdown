...
import { HUMAN_ONLY, RESULT_STATUSES, THREAD_KINDS } from './vocab.js';
...
export const handlers = {
  ...
  'POST /api/threads': (blueprint, _req, _url, body) => {
    ...
    return writes.openThread(blueprint, {
      kind,
      body: text,
      author: whoIsHere(blueprint).username,
      anchor: {
  ...
  'POST /api/draft': (blueprint, _req, _url, body) => {
    const { target = 'local', started, verdicts, threads = {}, discard } = body ?? {};
    // A draft is working state rather than a record, but it is stamped with a
    // name and the panel shows it back, so it takes the same answer as
    // everything else here rather than keeping a second one alive.
    const actor = whoIsHere(blueprint).username;
  ...
  'POST /api/walkdowns': (blueprint, _req, _url, body) => {
    const { target = 'local', results, roles } = body ?? {};
    /*
     * A walkdown is an acceptance - it fills a role's signature - so it asks
     * the same of the machine that a verify does: a person who wrote down who
     * they are. The actor used to arrive in the body and was only checked for
     * being non-empty.
     */
    const who = whoIsHere(blueprint);
    mustBeDeclared(who, 'a walkdown');
    const actor = who.username;
    /*
...
/*
 * WHO a browser write is recorded under, and the answer is never the request.
 *
 * `author` and `actor` used to arrive in the body and were passed straight
 * through. This server has no authentication and will never have any - it is
 * a localhost review server over one person's own blueprint - so a name in a
 * request is asserted, never proved, and an assertion nobody can check is the
 * same text field that `--actor` was at the CLI (n-0139). A judging agent
 * POSTed a status with a name it invented and the thread stood on disk
 * `verified_by` that name; the panel offered the same click under a login
 * name nobody had typed, because whoAmI() falls back to the identity the
 * server derived.
 *
 * So the server records under ITS OWN configured identity, exactly as the CLI
 * does. What a caller may still say is which hats it is signing in - roles
 * are a per-sitting choice a person makes, not a claim about who they are.
 */
const whoIsHere = (blueprint) => defaultActor(blueprint.codeRoot ?? blueprint.projectRoot);

/*
 * And the same accept gate the CLI applies. Removing the override is not
 * enough on its own: a machine whose config declares nobody still has a git
 * email and a login name to fall back to, and accepting work under a guess is
 * the finding n-0130 made and n-0143 found again on this door.
 */
function mustBeDeclared(who, what) {
  if (who.declared) return;
  throw new Error(
    `"${what}" is recorded under a person's name, and this machine only has a guess (${who.username}, from ${who.source}). Say who you are in ~/.walkdown/config.yml under \`identity:\` — a login name is not a decision`,
  );
}
...
export function threadAction(blueprint, id, action, body) {
  const who = whoIsHere(blueprint);
  if (action !== 'replies' && HUMAN_ONLY.includes(body.status)) mustBeDeclared(who, body.status);
  const thread =
    action === 'replies'
      ? writes.reply(blueprint, id, { author: who.username, body: body.body })
