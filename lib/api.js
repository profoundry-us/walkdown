...
import { normalizeRoles, normalizeSignatures, repoWeb } from './run-record.js';
...
export const handlers = {
  'GET /api/blueprint': (blueprint, _req, _url) => {
    ...
    return {
      // The BLUEPRINT's own name, from its walkdown.yml. It was called
      // `project` here while the panel had one word for two things (ADR 0001
      // §2); the spec file's own key is a separate question, parked in q-0270.
      blueprint: config.blueprint ?? 'walkdown',
      // Which of the listed blueprints this answer IS - the key a `?bp=` names
      // it by - so a page served under the server's default can still say
      // which one it was looking at when it links out (n-0297).
      key: blueprint.dir,
      ...
      panelHash: createHash('sha256')
        ...
        .digest('hex')
        .slice(0, 12),
      // Every blueprint this server lists, each naming the project it is in.
      blueprints: blueprint.listed,
      root: blueprint.root,
      targets,
      rows,
      drift,
  ...
  'GET /api/checks': (blueprint, _req, url) => {
    ...
    return {
      rule: ruleId,
      // Where to read the same lines on the web, when the tree is a GitHub
      // clone (n-0318). The panel builds the link; the server only knows.
      repo: repoWeb(root),
      checks: serve
  ...
  'POST /api/threads': (blueprint, _req, _url, body) => {
    const { kind = 'note', body: text, anchor = {}, url, reason = null, options = null } = body ?? {};
    const { said, added } = relayed(body);
    if (!THREAD_KINDS.includes(kind)) throw new Error(`kind must be ${THREAD_KINDS.join('|')}`);
    const via = provenance(body);
    // Why the note exists (ADR 0005 §1). The browser's doors file a person's
    // words - feedback unless the person says decision or request - and the
    // door validates the word; the default is decided where the record is
    // written, from who is acting.
    if (reason !== null && typeof reason !== 'string') throw new Error('reason must be a word');
    // A relayed note's body is what the person said.
    if (!(text || said) || typeof (text ?? said) !== 'string') throw new Error('body text required');
  ...
  'POST /api/draft': (blueprint, _req, _url, body) => {
    const { target = 'local', discard } = body ?? {};
    /*
     * The session's own fields come off the body through `sitting()` rather
     * than a list this handler keeps: a door that names its own subset is a
     * door that can quietly drop what the panel sent, which is exactly how a
     * sitting declared as eng and product came back from a reload as eng
     * alone (n-0230). One list, in lib/sitting.js, for every hop.
     */
    const session = sitting(body ?? {});
    // An emptied session and an explicit discard are the same thing: no
    // draft. Leaving a husk behind would haunt the next sitting.
    if (discard || !session.verdicts) {
      writes.discardDraft(blueprint, target);
      return { draft: null };
    }
    const rulesById = new Map(collectRules(blueprint.features).map(({ rule }) => [rule?.id, rule]));
    for (const [rule, status] of Object.entries(session.verdicts)) {
    ...
    // Validated the same way the run door validates it, so a draft cannot
    // hold a signature the ledger would later refuse.
    if (session.signatures) session.signatures = normalizeSignatures(session.signatures);
    // The actor is stamped by writes.js, like every other name in the ledger.
    return { draft: writes.saveDraft(blueprint, { target, ...session }) };
  },
  ...
  'POST /api/walkdowns': (blueprint, _req, _url, body) => {
    ...
    /*
     * No role, no walkdown (q-0312). An empty answer used to file a run under
     * no roles at all, which the ledger read as engineering's - the
     * historical default - so 29 rules carried an eng signature nobody gave
     * (q-0225). The panel asks who signs before a sitting starts; the door
     * asks again, because a record is forever.
     */
    if (!signedAs?.length && !(Array.isArray(signatures) && signatures.some((s) => s?.role)))
      throw new Error('a walkdown signs for at least one role — say who is signing before it is recorded');
    /*
      ...
      return {
        ...
        ...(rule.statement &&
          ['pass', 'fail', 'approved'].includes(r.status) && {
            statement_hash: formatHash(rule),
          }),
    ...
    return {
      ...
      // The threads this pass closed on the person's behalf (ADR 0005), so
      // the panel can say so beside the verdict rather than leave them to be
      // found later.
...
/** The per-thread mutations: POST /api/threads/<id>/(replies|status|later). */
export function threadAction(blueprint, id, action, body) {
  // Later is a person reordering their own queue: no words, no status, no
  // provenance - so it neither needs nor takes the fields the others do.
  if (action === 'later') return { thread: writes.defer(blueprint, id).thread };
  /*
  ...
  const { thread } =
    action === 'replies'
      ? writes.reply(blueprint, id, { body: body.body, via, asIs: body.as_is === true, attachments: body.attachments ?? null, ...relayed(body) })
      : writes.transition(blueprint, id, { status: body.status, reason: body.reason, via, chosen: chosenOf(body), attachments: body.attachments ?? null });
  return { thread };
}

/** The choice an answer took, as the caller states it: a short label, or nothing. */
function chosenOf(body) {
  const c = body?.chosen;
  if (c === undefined || c === null) return null;
  if (typeof c !== 'string' || !c.trim()) throw new Error('chosen names the option picked - a label, or leave it out');
  return c.trim();
}

/*
 * A person's words a machine is relaying, and what it added beside them.
 * Both are plain text or absent; the policy for what they mean lives in
 * lib/writes.js (whoseWords), and a door only carries them.
 */
function relayed(body) {
  const text = (v, name) => {
    if (v === undefined || v === null) return null;
    if (typeof v !== 'string') throw new Error(`${name} is text`);
    return v;
  };
  const said = text(body?.said, 'said');
  // A relay that sends `body` beside `said` means the body as its addition -
  // the one message has one set of the person's words, and the rest is the
  // machine's.
  return { said, added: text(body?.added, 'added') ?? (said ? text(body?.body, 'body') : null) };
}
