...
export const handlers = {
  ...
  'POST /api/draft': (blueprint, _req, _url, body) => {
    ...
    return { draft: writes.saveDraft(blueprint, { target, started, verdicts, threads }) };
  },
...
export function threadAction(blueprint, id, action, body) {
  ...
  const { thread } =
    action === 'replies'
      ? writes.reply(blueprint, id, { body: body.body })
      : writes.transition(blueprint, id, { status: body.status, reason: body.reason });
  return { thread };
}
