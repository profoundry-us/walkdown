 ...
 * compared the way `screenKey` compares it: fragment kept, query dropped. Within one blueprint
...
export function claimsOf(blueprint) {
  ...
  for (const [target, cfg] of Object.entries(targets)) {
    ...
    for (const screen of screens) {
      const ref = splitScreenRef(screen?.app?.path);
      if (!ref) continue;
      // An app path written as a whole URL names its own origin (#15) - a
      // stand-in on walkdown's server is not a page of the target's app.
      const at = ref.origin || origin;
      const path = ref.path + ref.fragment;
      out.push({ key: `${at}${path}`, origin: at, path, screen: screen.id, target });
    }
  }
  return out;
}
