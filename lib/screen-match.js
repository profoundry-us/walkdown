 ...
 *
 * An app path may also be a whole URL - a stand-in on walkdown's own origin
 * for a screen the app has not built yet, while base_url is the app (#15).
 * Its origin is kept apart from the path, so matching still reads the path
 * and a screen is still told apart from the same path on another origin.
 */
function splitScreenRef(ref) {
  if (!ref) return null;
  const whole = String(ref);
  const abs = whole.match(/^([a-z][a-z0-9+.-]*:\/\/[^/?#]*)(.*)$/i);
  const origin = abs ? abs[1].toLowerCase() : '';
  const s = abs ? abs[2] || '/' : whole;
  ...
  return {
    origin,
    path: q < 0 ? head : head.slice(0, q),
    query: q < 0 ? '' : head.slice(q),
    fragment,
  };
}
...
function screenKey(ref) {
  ...
  return parts ? parts.origin + parts.path + parts.fragment : null;
}
...
function scoreRef(ref, loc) {
  if (ref.origin && loc.origin && ref.origin !== String(loc.origin).toLowerCase()) return -1;
...
/*
 * Where a screen's app path is, against the target's base_url - resolved the
 * way a browser resolves a link, so a root path lands on the app and a whole
 * URL (a stand-in on walkdown's own server) lands where it says (#15). The
 * panel frames this and the server photographs it; they concatenated and
 * resolved respectively, and disagreed. One function now, for both.
 */
function appUrlOf(path, base) {
  if (!path || !base) return null;
  try {
    return new URL(String(path), String(base)).href;
  } catch {
