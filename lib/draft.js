...
export const draftPath = (drafts, target = 'local') => {
  if (!drafts)
    throw new Error(
      'No drafts directory — the entry names none and no home to derive one from. `walkdown where drafts` says what resolved.',
    );
  return join(drafts, `${String(target).replace(/[^\w.-]/g, '-')}.json`);
};

/** The draft for a target, or null. Unreadable or corrupt reads as none. */
export function readDraft(drafts, target = 'local') {
  // Reading is allowed to come up empty: a project with nowhere to keep
  // drafts simply has none, and saying so beats refusing the whole request.
 ...
 */
export function writeDraft(drafts, { target = 'local', actor, ...session }) {
  ...
  /*
   * Everything the caller may name comes through `sitting()` — the one list
   * of what a session is made of. This object used to spell those fields out
   * itself, one key at a time, which is how a draft handed `signatures` wrote
   * a file without them and a reload narrowed a two-role sitting to one
   * (n-0230). A field added to lib/sitting.js lands here with no edit.
   */
  const draft = {
    draft: true,
    target,
    actor: actor ?? null,
    updated: now,
    // `started` defaults, everything else is whatever the session carries.
    // (`sitting()` first, so a session with the key present but empty falls
    // back to the default instead of overwriting it with nothing.)
    ...{ started: now, ...sitting(session) },
  };
