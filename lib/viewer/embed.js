  ...
  function openThreadPopover(pin, dot) {
    ...
    overlay.innerHTML = `
      <div class="flex items-baseline gap-1.5">
        ...
        <span class="text-[11px] opacity-50">${pin.kind} · ${pin.status}</span>
      </div>
      <p class="mt-1 leading-relaxed">${pin.body ?? ''}</p>
      ${(pin.replies ?? []).map((r) => `<div class="mt-1.5 ml-2 border-l-2 border-base-300 pl-2">
        <span class="text-[11px] opacity-50">${r.author} · ${r.created}</span>
        <p class="leading-relaxed">${r.body}</p></div>`).join('')}
      <textarea class="textarea textarea-sm mt-2 h-16 w-full" placeholder="Reply…"></textarea>
      <div class="mt-2 flex gap-2">
        <button class="btn btn-xs btn-primary wd-primary">Reply</button>
        <button class="btn btn-xs btn-ghost wd-cancel">Close</button>
      </div>`;
