...
(() => {
  ...
  function openThreadPopover(pin, dot, pending = []) {
    ...
    overlay.innerHTML = `
      ...
      <div class="wd-stream mt-1 max-h-64 overflow-y-auto">${MSG.stream(pin, {
        pending, names: MSG.nameMap(identity),
      })}</div>
  ...
  function openForm(el, point) {
    ...
    overlay.style.left = `${Math.max(window.scrollX + 8,
      Math.min(point.x - 24, window.scrollX + window.innerWidth - 268))}px`;
  ...
  // --- vocab:start --- (generated by tools/sync-shared.mjs — edit lib/vocab.js) ---
  // ---- threads --------------------------------------------------------------
  ...
  if (framed) {
    ...
    const resolve = () => {
      ...
      ctx.pins = !hit ? [] : blueprint.threads.filter(
        (t) => t.anchor?.screen === hit.screen.id && !TERMINAL.includes(t.status)
