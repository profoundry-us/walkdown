...
(() => {
  ...
  side.style.cssText = `position:absolute; top:${TOP + GAP}px; right:${GAP}px; bottom:${GAP}px;
  ...
  side.className = 'flex flex-col overflow-hidden bg-base-100 text-base-content';
  ...
  function paintDesk(on) {
    ...
    root.style.margin = `${TOP + GAP}px ${W + GAP * 2}px ${GAP}px ${GAP}px`;
  ...
  function render() {
    ...
    side.innerHTML = `
      ...
      <div role="tablist" class="tabs tabs-border shrink-0">
        <button role="tab" class="tab${listTab === 'rules' ? ' tab-active' : ''}" data-tab="rules">Rules</button>
        <button role="tab" class="tab${listTab === 'screens' ? ' tab-active' : ''}" data-tab="screens">Screens</button>
      </div>
      ...
      <div class="flex min-h-0 flex-1 overflow-hidden">
        <div class="wdp-track flex min-h-0 flex-[0_0_200%] transition-transform duration-300 ease-out">
          <div class="wdp-pane flex min-h-0 w-1/2 flex-[0_0_50%] flex-col overflow-y-auto">${
            listTab === 'screens' ? screensPane() : listPane()}</div>
  ...
  function renderBar() {
    ...
    bar.innerHTML = `
      ...
      <span class="absolute left-1/2 flex -translate-x-1/2 items-center gap-2"
        ...
        <button class="btn btn-xs${strength === 100 ? ' btn-active' : ''}" data-surface="prototype"
        ...
        <button class="btn btn-xs${strength === 0 ? ' btn-active' : ''}" data-surface="app">App</button>
      </span>
  ...
  function renderGate() {
    ...
    if (phase === 'connect') {
      side.innerHTML = `
        <div class="flex flex-1 flex-col justify-center gap-3 p-5">
          ...
          <p class="text-[12.5px] leading-relaxed opacity-60">walkdown reads and writes blueprint
            files on disk, so it works through the local server that owns that folder.</p>
          ...
          <div class="flex items-center gap-2">
            ...
            <button class="btn btn-sm btn-primary" id="wdp-retry">Open</button>
          </div>
          <p class="text-[11.5px] opacity-40">Every blueprint under that folder shows up here.</p>
        </div>`;
      side.querySelector('#wdp-retry').onclick = () => {
        const at = side.querySelector('#wdp-server').value.trim();
    ...
    side.innerHTML = `
      <div class="border-b border-base-300 p-4">
        ...
        <p class="mt-1 text-[12.5px] leading-relaxed opacity-60">Found ${projects.length} under
          <span class="font-mono text-[11px]">${esc(SERVER)}</span>. Remembered for
          <b>${esc(location.origin)}</b>.</p>
      </div>
      <div class="flex-1 overflow-y-auto">
        ${projects.map((pr) => `
          <button class="block w-full border-b border-base-300 px-4 py-3 text-left hover:bg-base-200"
            data-pick="${esc(pr.id)}">
            <span class="block text-[13.5px] font-semibold">${esc(pr.name)}</span>
            <span class="mt-0.5 block text-[12px] leading-snug opacity-60">${
              esc(pr.description ?? 'No description \u2014 add one to this blueprint\u2019s walkdown.yml.')}</span>
            <span class="mt-1 block font-mono text-[10.5px] opacity-35">${esc(pr.id)}</span>
          </button>`).join('')}
      </div>`;
  ...
  setDocked(true);
  window.walkdownEmbed?.demoteBadge();
