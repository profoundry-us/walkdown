...
(() => {
  ...
  /*
   * What the rule search box says. Kept out here rather than read off the
   * input, because the pane is rebuilt wholesale on every render and a filter
   * whose only record was the DOM would forget itself the next time anything
   * else on the panel changed.
   */
  let ruleQuery = '';
   ...
   *
   * The pane is rebuilt wholesale on every render, and renders arrive
   * unbidden - the framed surface announcing itself is enough. So a disclosure
   * whose only record of being open was the DOM closed itself a frame later,
   * and the source, fetched meanwhile, was written into a node that had
   * already been thrown away: opened, it said "Loading…" forever (n-0084,
   * n-0057). Both the asking and the answer live here now, keyed by rule, so
   * moving to another rule collapses it again without anything to reset.
   */
  let srcOpenFor = null;                   // the rule whose source is open
  let srcCache = { rule: null, html: null };
  /*
  ...
  function buildDeskPanel() {
    deskPanel.innerHTML = `
      <div class="mb-2 flex items-center gap-2">
        ...
        <input id="wdp-set-actor" data-testid="settings.actor" class="input input-xs ml-auto w-36" placeholder="username"
          value="${esc(identityOverride.username ?? session?.actor ?? data?.identity?.username ?? '')}"
      ...
      <div class="mb-2 flex items-center gap-2">
        ...
        <input id="wdp-set-name" data-testid="settings.display-name" class="input input-xs ml-auto w-36" placeholder="optional"
          value="${esc(identityOverride.name ?? data?.identity?.name ?? '')}"
    ...
    act.onchange = () => {
      ...
      if (session) { session.actor = whoAmI(); saveSession(); }
      render();
    };
  ...
  function wireSearch() {
    ...
    if (!box) return;
    box.oninput = () => { ruleQuery = box.value; paintRules(); };
    box.onkeydown = (e) => {
      ...
      if (e.key !== 'Escape' || !ruleQuery) return;
      e.stopPropagation();
      ruleQuery = '';
      box.value = '';
      paintRules();
   ...
   */
  const checkRefs = (row) => {
    const recorded = [...new Set((data?.targets ?? []).flatMap((t) => row.cells?.[t]?.checks ?? []))];
    return recorded.length ? recorded : (data?.checkSource?.[row.rule] ?? []);
  };
  ...
  async function loadCheckSource(rule) {
    if (srcCache.rule === rule && srcCache.html) return;
    srcCache = { rule, html: null };
    let html;
    try {
          ...
          ${checkRefs(r).length ? `<!-- The steps are the rule; the source that checks them is a
            ...
            <details class="mt-2 rounded border border-base-300 bg-base-200/60 px-2 py-1 text-[11.5px]"
              ...
              <div class="wdp-check-src mt-1 opacity-70">${
                srcCache.rule === r.rule && srcCache.html ? srcCache.html : 'Loading…'}</div>
            </details>` : ''}</div>` : ''}
        <div>
  ...
  const whoAmI = () =>
    (identityOverride.username ?? session?.actor ?? data?.identity?.username ?? '').trim();
  /** The username a sitting's records will carry: the one it was started under. */
  const recordingHandle = () => (session?.actor ?? '').trim() || whoAmI();
  /** And how to read that handle - the same fallback, one place. */
  const recordingDisplay = () => {
    const full = (identityOverride.name ?? data?.identity?.name ?? '').trim();
  ...
  const names = () => MSG.nameMap({
    username: whoAmI(),
    name: (identityOverride.name ?? data?.identity?.name ?? '').trim(),
    handles: [...(data?.identity?.handles ?? []), session?.actor].filter(Boolean),
  });
  ...
  function wireThreads() {
    ...
    host.querySelectorAll('[data-checks]').forEach((el) => {
      el.ontoggle = () => {
        ...
        if (el.open === (srcOpenFor === rule)) return;
        srcOpenFor = el.open ? rule : null;
  ...
  const declaredAnchors = () =>
    new Set((data?.storyboard ?? []).flatMap((s) => s.anchors ?? []));
