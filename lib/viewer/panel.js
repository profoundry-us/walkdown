  ...
  function render() {
    ...
    side.innerHTML = `
      <div role="tablist" class="tabs tabs-box m-2 shrink-0">
        ${tab('rules', 'Rules')}${tab('screens', 'Screens')}${tab('blueprints', 'Blueprints')}
      </div>
  ...
  function renderBar() {
    ...
    bar.innerHTML = `
      ...
      <span class="ml-auto flex items-center gap-2">
        <button class="btn btn-xs ${pinning ? 'btn-warning' : 'btn-outline btn-primary'}" id="wdp-pin"
        ...
        <button class="btn btn-xs btn-outline btn-primary" id="wdp-undock" title="Put walkdown away">\u00d7</button>
      </span>`;
  ...
  function blueprintsPane() {
    return `
      <div class="px-3.5 pb-2 pt-1">
        <div class="mb-1 text-[11px] font-bold uppercase tracking-wider opacity-50">Folder</div>
        <div class="flex items-center gap-2">
          <input id="wdp-server" class="input input-xs flex-1" value="${esc(SERVER)}"
                 aria-label="walkdown server address">
          <button class="btn btn-xs btn-outline btn-primary" id="wdp-retry">Open</button>
        </div>
        <p class="mt-1.5 text-[11px] leading-relaxed opacity-40">Every blueprint under the folder
          <code>walkdown serve</code> was started in.</p>
      </div>
