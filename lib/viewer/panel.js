...
(() => {
  ...
  const PHOSPHOR = {
    'browser': '<path d="M216,40H40A16,16,0,0,0,24,56V200a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A16,16,0,0,0,216,40Zm0,16V88H40V56Zm0,144H40V104H216v96Z"/>',
    ...
    'folders': '<path d="M224,64H154.67L126.93,43.2a16.12,16.12,0,0,0-9.6-3.2H72A16,16,0,0,0,56,56V72H40A16,16,0,0,0,24,88V200a16,16,0,0,0,16,16H192.89A15.13,15.13,0,0,0,208,200.89V184h16.89A15.13,15.13,0,0,0,240,168.89V80A16,16,0,0,0,224,64ZM192,200H40V88H85.33l29.87,22.4A8,8,0,0,0,120,112h72Zm32-32H208V112a16,16,0,0,0-16-16H122.67L94.93,75.2a16.12,16.12,0,0,0-9.6-3.2H72V56h45.33L147.2,78.4A8,8,0,0,0,152,80h72Z"/>',
  ...
  // desk it sits on.
  deskPanel.style.cssText = `position:absolute; top:${TOP + GAP}px; left:${GAP * 3}px; display:none; pointer-events:auto;`;
  ...
  function hideApp(on) {
    ...
    // the ruling behind it. The bar stays: its buttons have to keep working
    // while you're peeking, and it draws no surface of its own to cover the
    // desk with.
    for (const el of [app, side]) if (el) el.style.opacity = on ? '0.1' : '';
  }
  ...
  function render() {
    ...
    renderBar();
    const TAB_ICON = { blueprints: 'folders', rules: 'checks', screens: 'browser' };
    const tab = (id, label) =>
      `<button role="tab" class="tab gap-1${listTab === id ? ' tab-active' : ''}" data-tab="${id}">
        ${icon(TAB_ICON[id], 'size-3')}${label}</button>`;
    side.innerHTML = `
  ...
  const GEAR = () =>
    `<button class="btn btn-xs btn-ghost" id="wdp-desk-btn" title="Tune the desk">${icon('gear', 'size-3.5')}</button>`;
  const wireGear = () => {
    const gear = bar.querySelector('#wdp-desk-btn');
