...
(() => {
  ...
  style.textContent = `
    ...
    /* The track is twice the panel's width; the slot clips it, or the offscreen
       pane paints over the app instead of sliding within the panel. */
    .wdp-slot { flex: 1; min-height: 0; overflow: hidden; display: flex; }
    /* flex-basis 200%, not flex:1 — inside a row flex slot, flex:1 means
       basis 0 and would override width, collapsing both panes to half each. */
    .wdp-track { flex: 0 0 200%; display: flex; min-height: 0; transition: transform .26s cubic-bezier(.4,0,.2,1); }
    .wdp-track.wdp-detail { transform: translateX(-50%); }
    .wdp-pane { flex: 0 0 50%; width: 50%; display: flex; flex-direction: column; min-height: 0; overflow-y: auto; }
    ...
    /* the prototype, ghosted over the running app. The stage carries the
       opacity and a checkerboard, so where the prototype does not reach you
       can see that it has ended rather than reading the app as design. */
    .wdp-stage { position: fixed; top: 0; left: 0; bottom: 0; right: ${W}px; z-index: 2147482000;
    ...
    .wdp-ghost { width: 100%; height: 100%; max-width: 100%; max-height: 100%;
      border: 0; background: #fff; box-shadow: 0 0 0 1px rgba(20,25,40,.14), 0 10px 40px rgba(20,25,40,.18); }
    .wdp-empty { color: #9ca3af; font-size: 12.5px; padding: 14px; }
    /* A session records under a name, so the name is on screen while it does. */
    .wdp-session { display: flex; align-items: center; gap: 7px; padding: 8px 14px;
      border-bottom: 1px solid #e5e7eb; background: #fdf6e8; font-size: 12px; color: #92610a; }
    .wdp-session input { font: inherit; font-size: 12px; width: 104px; padding: 3px 7px;
      border: 1px solid #e8d4ae; border-radius: 6px; background: #fff; color: #16181d; }
    .wdp-session .wdp-fin { margin-left: auto; border: 1px solid #d97706; background: #d97706; color: #fff;
      font: inherit; font-size: 12px; font-weight: 600; padding: 4px 11px; border-radius: 7px; cursor: pointer; }
    .wdp-toast { position: fixed; bottom: 18px; right: ${W + 18}px; z-index: 2147483001;
      background: #16181d; color: #fff; font: 13px/1.4 -apple-system, sans-serif;
      padding: 10px 15px; border-radius: 9px; box-shadow: 0 8px 28px rgba(20,25,40,.3); }
    .wdp-toast b { color: #6ee7b7; }`;
  ...
  function render() {
    ...
    host.innerHTML = `
      ...
      ${session ? `<div class="wdp-session">
        <span>Walkdown recording as</span>
        <input id="wdp-actor" value="${esc(session.actor)}" title="Recorded as the author of these verdicts">
        <span>${Object.keys(session.verdicts).length} judged</span>
        <button class="wdp-fin" id="wdp-finish">Finish</button>
      </div>` : ''}
      <div class="wdp-slot">
        <div class="wdp-track">
    ...
    if (track) {
      ...
      track.classList.toggle('wdp-detail', lastView === 'detail');
      void track.offsetWidth;
      track.style.transition = '';
      track.classList.toggle('wdp-detail', view === 'detail');
      lastView = view;
    }
  ...
  function footer() {
    ...
    return `<div class="wdp-foot">
      <button class="wdp-chip ${session ? 'wdp-on' : ''}" id="wdp-session">${session ? 'Discard' : 'Start walkdown'}</button>
      ...
      ${ghost ? `<input class="wdp-slider" id="wdp-op" type="range" min="0" max="100" value="${ghostOpacity * 100}">
        <select class="wdp-chip" id="wdp-gw" title="Width the prototype is drawn at">
  ...
  function wireFooter() {
    ...
    if (gw) gw.onchange = () => {
      ...
      const frame = ghost?.querySelector('.wdp-ghost');
  ...
  function setGhost(on) {
    ...
    // The stage owns the opacity so the backdrop fades with the prototype:
    // at full strength the app is properly covered, not blended into.
    ghost = document.createElement('div');
    ghost.className = 'wdp-stage';
    ghost.style.opacity = ghostOpacity;
    const frame = document.createElement('iframe');
    frame.className = 'wdp-ghost';
    if (ghostWidth) frame.style.width = `${ghostWidth}px`;
    frame.src = api('/prototype' + screen.prototype);
