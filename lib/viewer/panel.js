...
(() => {
    ...
    host.innerHTML = `
      ...
      <div class="flex min-h-0 flex-1 overflow-hidden">
        <div class="wdp-track flex min-h-0 flex-[0_0_200%] transition-transform duration-300 ease-out">
          <div class="flex min-h-0 w-1/2 flex-[0_0_50%] flex-col overflow-y-auto">${listPane()}</div>
          <div class="flex min-h-0 w-1/2 flex-[0_0_50%] flex-col overflow-y-auto">${detailPane()}</div>
        </div>
      </div>
      ${footer()}`;
  ...
  function detailPane() {
    ...
    // back to it. The arrows are vertical on purpose: ← already means "back to
    // the list" a couple of inches to the left on this same row, and one arrow
    ...
    return `
      ...
      <div class="flex flex-col gap-3 px-3.5 pb-3.5 pt-1">
        ...
        ${threads.length ? `<div><div class="${LBL} mb-1.5">Threads</div>
          ${threads.map((t) => `<div class="mb-1.5 rounded-r-box border-l-3 px-2.5 py-2 ${
            t.status === 'addressed' || t.status === 'answered'
              ? 'border-l-warning bg-warning/10' : 'border-l-base-300 bg-base-200'}">
            <div class="text-[11.5px] opacity-60"><b class="opacity-100">${esc(t.id)}</b> · ${esc(t.status)}</div>
