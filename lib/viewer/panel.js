...
Please report this to https://github.com/markedjs/marked.`,e){let i="<p>An error occurred:</p><pre>"+R$1(n.message+"",true)+"</pre>";return t?Promise.resolve(i):i}if(t)return Promise.reject(n);throw n}}};var M$1=new Q;function f$1(l,e){return M$1.parse(l,e)}f$1.options=f$1.setOptions=function(l){return M$1.setOptions(l),f$1.defaults=M$1.defaults,U(f$1.defaults),f$1};f$1.getDefaults=A$1;f$1.defaults=T;function bt(...l){return M$1.use(...l),f$1.defaults=M$1.defaults,U(f$1.defaults),f$1}f$1.use=bt;f$1.walkTokens=function(l,e){return M$1.walkTokens(l,e)};f$1.parseInline=M$1.parseInline;f$1.Parser=b$1;f$1.parser=b$1.parse;f$1.Renderer=P$1;f$1.TextRenderer=L$1;f$1.Lexer=x$1;f$1.lexer=x$1.lex;f$1.Tokenizer=y$1;f$1.Hooks=S$2;f$1.parse=f$1;f$1.options;f$1.setOptions;f$1.walkTokens;f$1.parseInline;b$1.parse;x$1.lex;
  ...
  const openQuestionOn = (rule) => asksOn(rule)[0] ?? null;
   ...
   */
  async function answerOnRule(rule, text, chosen = null) {
  ...
  async function laterOnRule(rule) {
    const q = openQuestionOn(rule);
    if (!q) return null;
  ...
  function signoffStack(acceptance, mine) {
    ...
    return b`<span class="grid shrink-0 grid-flow-col grid-rows-2 items-center justify-center gap-x-[3px]"
    data-testid="panel.rule-signoff" data-signoff="${all.map((a) => `${a.role}:${a.state}`).join(' ')}"
    data-columns="${cols}"
    >${all.map(
      (a, i) =>
        b`<span class="flex h-[9px] w-[9px] items-center justify-center ${
   ...
   */
  function tierMarks(row, mine = false, opts = {}) {
    /*
    ...
    return b`<span class="tooltip tooltip-right flex w-13 shrink-0 items-center justify-center gap-0.5 text-[12px] leading-none"
  ...
  function askOptions(asked) {
    if (!asked?.options?.length) return A;
    return b`<div class="mt-1.5 flex flex-col gap-1" data-testid="detail.ask-options">${asked.options.map(
    (o) => b`<button type="button" class="flex items-start gap-2 rounded border px-2 py-1 text-left ${S.askChoice === o.label ? 'border-primary bg-primary/15' : 'border-base-300 bg-base-100/60'}"
        ...
        <span><b class="block text-[11.5px]">${o.label}</b>${o.why ? b`<span class="block opacity-70">${o.why}</span>` : A}</span>
      </button>`,
  )}</div>`;
  }
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-3.5 pb-2" data-testid="thread.body">
      ...
      ${
        ...
        t.kind === 'question' && t.status === 'open'
          ? b`<div class="mt-3 rounded border border-primary/60 bg-primary/5 px-2 pt-2.5 pb-2 text-[11px] leading-snug" data-testid="thread.ask">
              <div class="wd-stream">${o$1(
  ...
  const dragFiles = (e) => [...(e.dataTransfer?.types ?? [])].includes('Files');
   ...
   * on the zone.
   */
  const dropCover = (key, line) =>
    S.dragFiles
      ? b`<div class="pointer-events-none absolute inset-0 z-[5] flex items-center justify-center rounded-box backdrop-blur-[1.5px] ${
      ...
      : A;
  function dropZone(key, take, allowed = () => true) {
  ...
  function askCard(r, asked, open) {
    ...
    return b`<div class="relative mt-3 mb-1.5 rounded border border-primary/60 bg-primary/5 px-2 pt-2.5 pb-2 text-[11px] leading-snug"
    ...
    <span class="absolute -top-[7px] right-2 flex items-center gap-1 rounded bg-base-100 px-1 text-[9px] leading-[14px] opacity-70" data-testid="detail.ask-count" title="${total} ask${total === 1 ? '' : 's'} on this rule">
      ${at} of ${total}
      ${Array.from({ length: total }, (_, i) => b`<i class="inline-block h-[6px] w-[6px] rounded-full ${i < done ? 'bg-success' : i === done ? 'bg-primary' : 'bg-base-300'}"></i>`)}
    </span>
    <div class="wd-stream max-h-56 overflow-y-auto" @click=${open}>${o$1(
    ...
    <div class="mt-1 flex flex-wrap items-center gap-1" data-testid="detail.ask-actions">
      ...
      ${
        queue.length > 1
          ? b`<button class="btn btn-xs btn-ghost" data-v="later" title="Put this one off: the next ask comes up, and this comes round again after it"
  ...
  function historyPane() {
    ...
    return b`<div class="flex items-center gap-2 px-2 pt-2">
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-3.5 pb-3">
      ${
        messages.length
          ? b`<div class="wd-stream" data-testid="history.stream" @click=${openTagged}>${o$1(
              MSG.stream({ replies: messages }, { rules: known, pending: note ? (pendingReplies.get(note.id) ?? []) : [], names: names() }),
            )}</div>`
          : b`<p class="text-[12.5px] opacity-50">Nothing said on this rule yet.</p>`
      }
    </div>`;
  }
  ...
  function conversation(r, picked) {
    ...
    return b`<div class="relative -mx-3.5 border-t border-base-300 px-3.5 pt-2 ${dropZoneClass('rule')}" data-testid="detail.conversation"
    ...
    <div class="${LBL} mb-1 flex items-center">Conversation${all.length ? b` <span class="ml-1 font-normal normal-case tracking-normal opacity-70">\u00b7 ${all.length} thread${all.length === 1 ? '' : 's'}</span>` : A}${
      messages.length > 1
        ? b`<button class="btn btn-ghost btn-xs ml-auto h-5 min-h-0 px-1.5 font-normal normal-case tracking-normal text-primary" data-testid="detail.history-open"
  ...
  const SCREENS_VIEW_KEY = 'walkdown:screens-view';
  async function loadScreensView() {
   ...
   */
  function viewTabs() {
    const tab = (id, label) => b`<button role="tab" class="tab tab-xs ${S.screensView === id ? 'tab-active' : ''}"
    ...
    return b`<div role="tablist" class="tabs tabs-boxed tabs-xs mx-3.5 mb-1 w-fit" data-testid="panel.screens-views">
  ...
  function render() {
    ...
    <div class="flex min-h-0 flex-1 overflow-hidden">
      <div class="wdp-track flex min-h-0 min-w-0 flex-[0_0_300%] transition-transform duration-300 ease-out">
        ...
        <div class="flex min-h-0 w-1/3 flex-[0_0_33.3333%] flex-col overflow-hidden"
          data-testid="${onThreads ? A : S.view === 'history' ? 'history.panel' : 'thread.panel'}">${
            onThreads ? A : S.view === 'history' ? historyPane() : threadPane()
          }</div>
      </div>
    </div>
    ...
    ${
      S.listTab === 'rules'
        ? b`<div class="flex shrink-0 items-center justify-between gap-2 border-t border-base-300 px-3.5 py-2 text-xs" data-testid="panel.counts">
      <span class="flex items-center gap-2">
