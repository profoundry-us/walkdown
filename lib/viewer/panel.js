...
(() => {
  ...
  /*
   * A thread reads as a conversation: the opening note is the first message,
   * not a body with a list of annotations under it. Collapsed it is a line of
   * provenance and the note; open it is the stream, a standing composer, and
   * the transitions this state allows - feedback gets answered where it is
   * read, without leaving the app under review.
   */
  function threadCard(t) {
    ...
    // The line that closed a thread. Terminal is terminal - the lifecycle has
    // no way back from here, so the card offers none.
    const ended = TERMINAL.includes(t.status) ? replies.at(-1) : null;
    return `<div class="mb-1.5 overflow-hidden rounded-r-box border-l-3 ${
      waiting ? 'border-l-warning bg-warning/10' : 'border-l-base-300 bg-base-200'}">
      <button class="flex w-full items-center gap-1.5 px-2.5 py-1.5 text-left text-[11.5px]" data-thread="${esc(t.id)}">
        <b>${esc(t.id)}</b>
        <span class="badge badge-xs ${CHIP[t.status] ?? 'badge-ghost'}">${esc(t.status)}</span>
        <span class="opacity-40">${esc(t.kind)}</span>
        ${!open && replies.length
          ? `<span class="opacity-40">${replies.length} ${replies.length === 1 ? 'reply' : 'replies'}</span>` : ''}
        ${unread ? `<span class="badge badge-xs badge-error">${unread} new</span>` : ''}
        <span class="ml-auto opacity-40">${open ? '▾' : '▸'}</span>
      </button>
      ${open ? `
        <div class="px-2.5 pb-1">${MSG.stream(t, {
          seenAt: seenAtOpen[t.id] ?? null,
          rules: (data?.rows ?? []).map((r) => r.rule),
          pending: pendingReplies.get(t.id) ?? [],
        })}</div>
        ${ended ? `<div class="mx-2.5 mb-1.5 flex items-center gap-1.5 rounded border border-success/40 px-2 py-1 text-[11px]">
          <span class="text-success">✓</span> ${esc(t.status === 'waived' ? 'Waived' : t.status)}${
            ended.author ? ` by <b>${esc(ended.author)}</b>` : ''} · ${esc(MSG.ago(ended.created))}</div>` : ''}
        ${sketch?.proposed ? `<div class="px-2.5"><button class="btn btn-xs btn-outline mb-1.5 w-full" data-sketch="${esc(t.anchor.screen)}">
          ⚠ View the proposed sketch</button></div>` : ''}
        <!-- The composer stays put: type, press Enter, the message is there.
             The name is not asked for again - it is whoever you are recording
             as, changed in Settings like everywhere else. -->
        <div class="border-t border-base-300 p-1.5">
          <textarea id="wdp-note" rows="2" class="textarea textarea-xs w-full resize-none"
        ...
        </div>` : `<div class="px-2.5 pb-2 text-[12.5px] leading-snug opacity-80">${
          esc(t.body).replace(/\s*\n+\s*/g, ' ')}</div>`}
    </div>`;
  }
  ...
  function wireThreads() {
    host.querySelectorAll('[data-thread]').forEach((el) => {
      el.onclick = () => {
        ...
        openThread = openThread === id ? null : id;
        threadNote = '';
        if (openThread) markSeen(openThread);
        render();
        // Land where the reading resumes: on the first unread message if there
        // is one, and otherwise at the newest, never at the top of an exchange
        // you have already read.
        if (openThread) {
          const card = host.querySelector(`[data-thread="${openThread}"]`)?.parentElement;
          const mark = card?.querySelector('.wd-new') ?? card?.querySelector('#wdp-note');
    ...
    host.querySelectorAll('[data-thread-ref]').forEach((el) => {
      el.onclick = (e) => {
        ...
        markSeen(id);
        const t = (data.threads ?? []).find((x) => x.id === id);
        selected = t?.anchor?.rule ? data.rows.find((r) => r.rule === t.anchor.rule) ?? null : null;
        view = 'detail';
        render();
      };
    });
