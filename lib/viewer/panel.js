...
(() => {
  ...
  function render() {
    ...
    side.innerHTML = `
      ${cfg.buildHash && data.panelHash && cfg.buildHash !== data.panelHash
        ? `<div class="shrink-0 border-b border-base-300 bg-error/10 px-3.5 py-2 text-[11.5px] leading-snug" data-testid="panel.stale">
      ...
      ${session ? `<div class="flex items-center gap-2 border-b border-base-300 bg-warning/10 px-3.5 py-2 text-xs" data-testid="panel.actor">
        ...
        <button class="btn btn-xs btn-warning" data-testid="panel.finish" id="wdp-finish">Finish</button>
      </div>` : ''}
      ...
      <div class="flex min-h-0 flex-1 overflow-hidden">
        <div class="wdp-track flex min-h-0 flex-[0_0_300%] transition-transform duration-300 ease-out">
          <div class="wdp-pane flex min-h-0 w-1/3 flex-[0_0_33.3333%] flex-col overflow-y-auto"
            ...
            : listPane()}</div>
          <div class="wdp-pane flex min-h-0 w-1/3 flex-[0_0_33.3333%] flex-col overflow-y-auto">${detailPane()}</div>
    ...
    if (track) {
      ...
      const AT = { list: '0%', detail: '-33.3333%', thread: '-66.6667%' };
  ...
  function threadPane() {
    ...
    return `
      <div class="flex items-center gap-1 px-2 pt-2">
        <button class="wdp-thread-back btn btn-ghost btn-xs text-primary">← ${
          esc(row ? shortName(row) : 'All rules')}</button>
        <span class="ml-auto flex items-center gap-1 pr-1.5 text-[11px]">
          <b class="opacity-60">${esc(t.id)}</b>
          <span class="badge badge-xs ${CHIP[t.status] ?? 'badge-ghost'}">${esc(t.status)}</span>
        </span>
      </div>
      ${where ? `<div class="px-3.5 pb-1 text-[11px] opacity-45">${where}</div>` : ''}
      <div class="min-h-0 flex-1 overflow-y-auto px-3.5 pb-2">
        ${MSG.stream(t, {
  ...
  function openThreadView(id) {
    ...
    const pane = host.querySelectorAll('.wdp-track > div')[2];
  ...
  function wireThreads() {
    ...
    if (tback) tback.onclick = () => {
      ...
      view = t?.anchor?.rule && selected ? 'detail' : 'list';
      openThread = null;
      render();
    };
    const note = host.querySelector('#wdp-note');
  ...
  function goTo(screen, surface = pageSurface()) {
     ...
     */
    if (!FRAMED) {
      if (!sameAddress(location.href, url)) location.assign(url);
      return true;
    }
  ...
  async function start() {
    ...
    if (!BP && projects.length > 1) {
      try {
        ...
        const asking = FRAMED ? frameUrl : location.href;
        const whose = asking
