...
(() => {
  ...
  async function restoreSession() {
    if (session) return;
    const saved = await store.get(SESSION_KEY()).catch(() => null);
  ...
  // An unfinished session is real judging work: it lives in storage from the
  // first verdict, so a page or extension reload - which the staleness banner
  // itself prescribes - never eats it silently.
  const SESSION_KEY = () => `walkdown:session:${BP}`;
  const saveSession = () => store.set(SESSION_KEY(), session && {
