...
import { afterPointer, POINTER_BEGIN, POINTER_END } from './init.js';
import {
  canon,
  claimHome,
  registryPath,
 ...
 * need not share one. */
/*
 * Relocate a directory, whatever stands between here and there.
 *
 * `renameSync` is the fast path and fails in two ordinary ways. EXDEV is the
 * destination being on another volume - an external drive, a mounted share,
 * the most plausible place for records leaving a repository. ENOTEMPTY and
 * EEXIST are a destination that already exists with something in it, which for
 * `walkdown move` means a directory holding nothing but a .DS_Store: the
 * caller's own guard ignores dotfiles, quite reasonably, and then rename would
 * not.
 *
 * Both fall back to copy-then-remove, and cpSync merges into whatever is
 * already there rather than refusing. Exported because `walkdown move` used a
 * bare renameSync and died with a raw EXDEV stack trace on the same hop this
 * function has always survived (n-0185) - one implementation, both callers.
 */
...
export function moveDir(from, to) {
  try {
    ...
    if (!['EXDEV', 'ENOTEMPTY', 'EEXIST'].includes(e.code)) throw e;
  }
  /*
   * What is at the destination BEFORE we write anything, so that a copy which
   * stops part way can be undone without touching a thing that was already
   * standing there. A name that appears afterwards and was not in this set is
   * ours and is debris; a name that was already there is somebody else's, and
   * their file is not ours to delete on our way out.
   *
   * Null means the directory itself is ours - we are about to create it - and
   * the whole thing comes away.
   */
  const before = existsSync(to) ? new Set(readdirSync(to)) : null;
  /*
   * The one case this cannot fully undo: a name we are copying that is ALREADY
   * a directory there, which cpSync merges into. Only a dotfile directory can
   * reach it (the caller's guard refuses a destination holding records, and
   * ignores dotfiles), and removing it would take their files with ours. So it
   * is reported rather than guessed at.
   */
  const merged = before
    ? readdirSync(from).filter((n) => before.has(n))
    : [];
  try {
    /*
     * Verbatim, because the copy path must be as faithful as the rename it
     * stands in for. cpSync resolves symlinks by default, so a record dir
     * holding `../real.json` arrived pointing at an absolute path back into
     * the directory it had just left, and no longer resolved - a file rewritten
     * to suit its new address, which is the one thing moving must never do
     * (n-0195). Timestamps for the same reason: rename keeps them, so this
     * keeps them.
     *
     * `force: false` for the third face of the same promise: moving must not
     * write over a file at the destination either. Only dotfiles can reach it -
     * the caller's guard refuses a destination holding records, and ignores
     * dotfiles on purpose - and quietly replacing somebody's is still a write
     * nobody asked for (n-0196).
     */
    cpSync(from, to, {
      recursive: true,
    ...
    /*
     * The copy's own failures, which moveDir used to have none of: it catches
     * what it needs to CHOOSE the copy and nothing the copy raises. A volume
     * that fills, a record it may not read, a destination parent it may not
     * write - each died as a stack trace and left a half-copied ledger behind,
     * so the obvious retry was then refused by the debris of the attempt that
     * failed (n-0196).
     *
     * Nothing is ever lost here: the source is removed only after the copy
     * returns. So the whole job is to take our own debris away and say what
     * happened in a sentence.
     */
    let cleaned = false;
    try {
      if (before === null) rmSync(to, { recursive: true, force: true });
      else
        for (const name of readdirSync(to))
          if (!before.has(name)) rmSync(join(to, name), { recursive: true, force: true });
      cleaned = !merged.length;
    } catch {
      /* said below, rather than thrown over the top of the real failure */
    }
    throw new MoveFailed(
      `copying ${from} to ${to} stopped part way (${e.code ?? e.message}). ` +
        `Nothing was lost — every record is still at ${from}, and nothing has been written down. ` +
        (cleaned
          ? 'The partial copy has been taken away, so this can be retried once the cause is fixed.'
          : `Most of the partial copy has been taken away, but ${merged.join(', ') || to} was already ` +
            'there and may hold some of it; check that before retrying.'),
      { cause: e },
    );
  }
...
export function removePointer(file) {
  ...
  if (to < 0) return 'kept';
  // The same boundary `placePointer` uses - taking the block out is walkdown
  // editing this file too, and it owned no more of it going than coming.
  const { after, kept } = afterPointer(text, to);
  const rest = text.slice(0, from) + kept + text.slice(after);
  if (!rest.trim()) {
    unlinkSync(file);
    return 'deleted';
  }
...
export function relocateHome({ loc, root, to }) {
   ...
   */
  for (const file of [registryPath(), join(root, '.walkdown', 'config.yml')]) {
   ...
   * and the personal row is written INTO on the way back out (rememberBlueprint).
  ...
  const written = rememberBlueprint({
    id: String(entry.id),
    root,
    homeDir: claim.dir,
    home: claim.home,
    inRepo: to === 'repo',
    moved: to === 'repo' ? {} : outside,
    by: 'move',
  });
  /*
   * The registry row follows the home either way (ADR 0003). Into the
   * repository, the committed file gets the manifest row and this machine's
   * row is re-pointed at the new home - with the port and the moved evidence
   * it was carrying, which a committed file cannot hold. It used to be
   * reduced to {id, roots, targets} in config.yml, the override shape, and
   * the way back out could not find it (n-0171); now there is one row and it
   * is written into.
   */
  if (to === 'repo')
    rememberBlueprint({
  ...
  /*
   * The old manifest row, gone - matched by the spec path it named, never
   * by id. Into the repository there is nothing to drop: the registry row
   * was re-pointed above, and config.yml registers nothing (ADR 0003). And
   * the repository's `.walkdown`, when it declares nothing any more, is
   * taken away entirely, so a clone does not find a manifest for a project
   * that has left.
   */
  const gone = [];
  if (to !== 'repo') {
    ...
    const left = existsSync(repoCfg) ? parseDocument(readFileSync(repoCfg, 'utf8')).get('blueprints')?.items?.length : 0;
    if (!left) {
...
export function tracking(loc) {
  ...
  const view = root && loc.blueprint ? gitView({ root, paths }) : { repo: false, kinds: {} };
  if (!loc.blueprint) return { view, words: '—', why: 'not a blueprint', findings };
