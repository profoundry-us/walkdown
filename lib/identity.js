...
import { configPath, readUserConfig } from './locations.js';
import { readerZone } from './time.js';
...
export function defaultActor(cwd) {
   ...
   */
  const declared = readUserConfig().config.identity ?? null;
  /*
   * A declaration is a plain string. A list or a number under `username:`
   * is a typo in the one file that must be right, and it used to be a raw
   * TypeError from here - which took down GET /api/blueprint and with it
   * the whole panel, with no word about which file to fix (n-0148). It is
   * read as nothing said, and `problem` names the file in the same breath
   * the accept gate uses, so the refusal that follows points at the line.
   */
  const said = spoken(declared?.username);
  const name = spoken(declared?.name).value || gitConfig(cwd, 'user.name');
  const explicit = said.value || gitConfig(cwd, 'user.username') || gitConfig(cwd, 'github.user');
  ...
  const problem = said.problem
    ? `\`identity.username\` in ${configPath()} is ${said.problem}, not a plain string, so it counts as nobody`
    : null;
  /*
   * The zone times are READ in, declared beside the name because it is a
   * fact about the person sitting here, not about any record - records
   * carry UTC and nothing else (n-0290). Unsaid, it is the machine's.
   */
  const zone = readerZone(declared?.timezone);
  return {
    ...
    username,
    name,
    source: said.value ? 'config' : explicit || local ? 'git' : 'os',
    name_source: spoken(declared?.name).value ? 'config' : name ? 'git' : null,
    timezone: zone.zone,
    timezone_source: zone.source,
    timezone_problem: zone.problem ? `${zone.problem} (${configPath()})` : null,
    /*
     * Whether the person WROTE this down, which is the question the accept
     * gate asks. A guess is not a signature.
     */
    declared: Boolean(said.value),
    // What is wrong with the declaration, when something is; null when the
    // file is fine or simply silent.
    problem,
    /*
     * WHICH file this was read from, so the refusals can name it.
     *
     * Every sentence telling somebody to write their identity down used to
     * say `~/.walkdown/config.yml` as a literal, and the file actually read
     * is configPath() - which honours WALKDOWN_HOME. Under a redirected home
     * the instruction pointed at a file nothing reads (n-0204). The server
     * knows the answer and the browser cannot work it out, so it travels in
     * the payload with the rest of the identity.
     */
...
/** A config value as something a person said: its trimmed string, or what it is instead. */
function spoken(v) {
  if (v === undefined || v === null) return { value: '', problem: null };
  if (typeof v === 'string') return { value: v.trim(), problem: null };
  const what = Array.isArray(v) ? 'a list' : v instanceof Map ? 'a map' : `a ${typeof v}`;
  return { value: '', problem: what };
}
