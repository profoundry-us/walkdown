 ...
 */
export function transitionThread(blueprint, id, { status, actor, reason, via = null, chosen = null, attachments = [] }) {
  const file = threadFile(blueprint, id);
  const t = parse(readFileSync(file, 'utf8'));
  ...
  // The reason is a reply, and carries pictures the way a reply does: a
  // fail's why reopens the note with its reason, and the picture dropped
  // beside that why went nowhere (Topher, 2026-09-21, n-0328).
  if (reason?.trim())
    (t.replies ??= []).push({
