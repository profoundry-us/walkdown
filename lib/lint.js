  ...
  if (blueprint.at?.project) {
    ...
    if (at.spec.path && basename(at.spec.path) !== HOME_LAYOUT.spec)
      add(
        ...
        cfg,
        at.id,
        `\`${at.id}\`'s spec ${at.spec.path} is not laid out as a home — every blueprint lives in one (${HOME_LAYOUT.spec}/ with ${KINDS.join(', ')} beside it): \`walkdown init\` lays one out, \`walkdown project add <home>\` lists one.`,
      );
  ...
  for (const r of blueprint.at?.config?.refused ?? [])
    if (r.from === 'personal')
      add(
        ...
        r.id,
        `\`${r.id}\` declares spec ${r.spec}, which lies under ${r.under} — that .walkdown answers for it; stand in that checkout, declare it there, or list a throwaway copy with \`walkdown project add --ephemeral\``,
      );
