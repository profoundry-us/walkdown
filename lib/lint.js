...
import { canon, escapingLinks, expand, HOME_LAYOUT, KINDS, walkdownRoot } from './locations.js';
...
export function lint(blueprint, { checks = true } = {}) {
  ...
  if (cfgFile && existsSync(cfgFile)) {
    let entries = [];
    try {
      entries = parse(readFileSync(cfgFile, 'utf8'))?.blueprints ?? [];
    } catch {
      entries = [];
    }
    for (const e of entries) {
      ...
      if (theirs && canon(theirs) !== canon(wd))
        add(
          ...
          cfgFile,
          String(e.id ?? e.spec),
          `\`${e.id ?? '?'}\` declares spec ${e.spec}, which lies under ${theirs} — that .walkdown answers for it; declare it there, or list a copy with \`walkdown blueprint add --ephemeral\``,
        );
    }
  }
  ...
  if (blueprint.at?.blueprint) {
    ...
    const cfg = at.config.matchedIn === 'repo' ? at.config.repo?.path : at.config.path;
    if (at.spec.path && basename(at.spec.path) !== HOME_LAYOUT.spec)
      add(
        ...
        cfg,
        at.id,
        `\`${at.id}\`'s spec ${at.spec.path} is not laid out as a home — every blueprint lives in one (${HOME_LAYOUT.spec}/ with ${KINDS.join(', ')} beside it): \`walkdown init\` lays one out, \`walkdown blueprint add <home>\` lists one.`,
      );
  ...
  /*
   * The same crossing in the PERSONAL file, which lint never read.
   *
   * The committed half is checked above by reading the answering `.walkdown`'s
   * own config. Written into ~/.walkdown/config.yml instead, the identical row
   * was honoured by every reader - listed, resolved, served, and written to -
   * while lint stayed silent on both sides of the repository (n-0188). The
   * resolver sets it aside now; this is the half that says so out loud, from
   * the list the resolver already hands over.
   */
  for (const r of blueprint.at?.config?.refused ?? [])
    if (r.from === 'personal')
      add(
        'error',
        'locations',
        blueprint.at.config.path,
        r.id,
        `\`${r.id}\` declares spec ${r.spec}, which lies under ${r.under} — that .walkdown answers for it; stand in that checkout, declare it there, or list a throwaway copy with \`walkdown blueprint add --ephemeral\``,
      );
