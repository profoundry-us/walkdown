...
export function lint(blueprint, { checks = true } = {}) {
  ...
  for (const { file, story, rule } of rules) {
    ...
    for (const v of declaredVerify) {
      if (v === 'human')
        ...
        add('error', 'schema', file, id, `unknown verify type "${v}" (expected ${DECLARABLE_VERIFY.join('|')})`);
    }
    ...
    if (rule.signoff != null) {
      ...
      if (!declaredSignoff.length)
        add('warn', 'signoff', file, id, 'signoff is empty — omit it to mean `[eng]`, which is what it means anyway');
      else if (!declaredSignoff.includes('eng'))
        add('warn', 'signoff', file, id,
  ...
  for (const { file, data: run } of runs) {
    ...
    if (run?.roles != null && !Array.isArray(run.roles))
      add('warn', 'runs', file, id, 'roles is not a list — the run reads as signed by nobody in particular');
    else
      for (const role of run?.roles ?? [])
        if (!ROLES.includes(role))
          add('warn', 'runs', file, id,
