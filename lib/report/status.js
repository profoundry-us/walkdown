...
const cellText = (cell, withActor = false) => {
  ...
  /*
   * A star on a pass that predates the fix claimed on its rule. The verdict
   * stands - it was earned, and this is not staleness - so it keeps its glyph
   * and its colour; what the star says is that it cannot have seen the fix,
   * and the queue below the table says which thread and what to do.
   */
  return `${glyph} ${label}${cell.unjudgedFix ? '*' : ''}`;
};
