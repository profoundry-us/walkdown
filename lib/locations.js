...
export function readUserConfig({ cwd = process.cwd() } = {}) {
   ...
   * it, `where --project` resolved the other pack's spec, runs, threads and
...
export function resolveLocations({
  ...
  const matched = want
    ? (() => {
         ...
         * contains cwd, exactly as it is without `--project`: mono/app and
         * mono/app/packs/app both contain the pack, and the first same-id
         * row whose roots did was the root's, so `thread new --project app`
  ...
  if (!spec) {
    ...
    if (at)
      spec = {
        path: null,
        why: `nothing declares ${expand(at, cwd)} — every blueprint walkdown answers for is written down; \`walkdown init\` starts one, \`walkdown project add <home>\` lists one`,
      };
    else if (!homeFor)
      spec = {
        path: null,
  ...
  for (const kind of KINDS) {
    ...
    out[kind] = homeFor
      ...
      : {
          path: null,
          why: `the entry names no ${kind} and no home — walkdown writes both; list the project again with \`walkdown project add\``,
        };
  }
