...
import { findBlueprintDir } from './blueprint.js';
...
/** The project entry whose `roots` contain this directory, if any. */
function projectFor(cwd, config) {
  const here = resolve(cwd);
  for (const p of config.projects ?? []) {
    for (const root of [p.roots ?? []].flat()) {
      const r = expand(root);
      if (r && (here === r || here.startsWith(r + '/'))) return p;
    }
  }
  return null;
}
...
export function resolveLocations({ cwd = process.cwd(), dir = null, overrides = {} } = {}) {
   ...
   */
  const project = dir
    ? (config.projects ?? []).find((p) => p.spec && expand(p.spec) === expand(dir, cwd)) ?? null
  ...
  if (dir) spec = { path: expand(dir, cwd), why: 'named on the command line' };
    ...
    const found = findBlueprintDir(cwd);
    if (found) spec = { path: resolve(found), why: 'found in the working tree' };
  }
  ...
  for (const kind of KINDS) {
    ...
    if (defaults[kind]) {
      out[kind] = { path: expand(String(defaults[kind]).replace('{id}', id)), why: 'the config default, outside the repository' };
      continue;
    }
    out[kind] = { path: builtIn(kind), why: 'the built-in default, outside the repository' };
  }
