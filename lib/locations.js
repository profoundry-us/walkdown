 ...
 */
/*
 * TWO WAYS TO BE ALREADY LISTED, because there are two shapes of ask.
 *
 * By spec, when the caller knows where the blueprint is - `init --in-repo`,
 * whose spec is `<root>/blueprint` before anything is built.
 *
 * By root, when it does not, which is init's DEFAULT shape: the spec lives
 * inside the home, so the home has to be chosen before the spec exists and
 * cannot be the thing that identifies the project. With `spec: null` the first
 * test can never fire, so `init` twice in one repository claimed a fresh name
 * each time - and the uniqueness loop bumped past the existing home precisely
 * because that directory was there, which is the claim from the first run
 * doing its job. Two entries with the same `roots`, the second's blueprint
 * read by nothing, while init told the person to go fill it in (n-0145).
 *
 * The root match is EXACT, never containment. Standing at a listed root again
 * is the same project; standing in a subdirectory of one is a different
 * question - whether a pack inside a listed repository is its own project is a
 * decision somebody makes, not one a claim should make quietly (n-0135).
 *
 * @param {{ id: string, spec?: string | null, root?: string | null, cwd?: string }} ask
 * @returns {{ id: string, dir: string, claimed: boolean }}
 */
export function claimHome({ id, spec = null, root = null, cwd = process.cwd() }) {
  const listed = readUserConfig({ cwd }).config.projects ?? [];
  const at = root && expand(root, cwd);
  const already =
    (spec && listed.find((p) => p?.spec && expand(p.spec) === expand(spec, cwd))) ||
    (at &&
      listed.find((p) => [p?.roots ?? []].flat().some((r) => r && expand(r) === at)));
...
export function resolveLocations({ cwd = process.cwd(), dir = null, overrides = {} } = {}) {
  ...
  const homeFor = (() => {
    ...
    const key = slug(project.id);
    const legacyHome = join(home, 'projects', key);
    return existsSync(legacyHome)
      ? {
          dir: legacyHome,
          why: 'the built-in default, outside the repository (legacy home — `walkdown where --fix` folds it into the config)',
        }
      : { dir: join(home, 'blueprints', key), why: 'the built-in default, outside the repository' };
  })();
  ...
  if (!spec) {
    ...
    if (!homeFor)
      spec = {
        path: null,
        why: 'nothing declares this directory — `walkdown init` starts a project, or name one with --dir',
      };
    else {
