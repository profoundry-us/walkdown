 ...
 * whole of claim + scaffold + write and rememberProject then asks for it
...
export function rememberProject({
  ...
  if (!homeDir) throw new Error('every blueprint lives in a home — rememberProject needs the directory the layout derives from');
  /*
  ...
  try {
     ...
     */
    const mine = root ? rowAbout(projects.items ?? [], { id, root, base }) : null;
    if (mine) {
...
export function readUserConfig({ cwd = process.cwd() } = {}) {
   ...
   * file instead, the identical row was honoured everywhere: `projects` listed
  ...
  const personalProjects = (personal.config.projects ?? []).map(anchored).filter((p) => {
    /*
   ...
   */
  const committed = (shared.projects ?? []).filter((p) => {
  ...
  for (const p of personalProjects) {
    ...
    if (!about(p, prev)) {
       ...
       * repository's.
       */
      const theirs = projectFor(cwd, { projects: [prev] });
      const its = projectFor(cwd, { projects: [p] });
...
export function resolveLocations({
  ...
  const matched = want
    ? (() => {
        ...
        const p =
          projectFor(cwd, { projects: named })?.project ??
  ...
  if (!spec) {
    ...
    if (at)
      ...
      spec = {
        path: null,
        why: want
          ? `no project \`${want}\` — \`walkdown blueprints\` lists them`
