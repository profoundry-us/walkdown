...
export function rememberLocation(loc, kind, to) {
  ...
  if (!doc.has('blueprints')) doc.set('blueprints', doc.createNode([]));
  const listed = doc.get('blueprints');
  const items = listed.items ?? [];
  /*
   * The personal row for this entry: same id, and about this checkout - the
   * merge's own test (rowAbout): a root that IS the code root, a pure
   * override, or an ephemeral copy of this very spec. Containment in either
   * direction is not asked. It was, and a move at a repository's root
   * recorded the root's evidence on the row of the pack nested inside it,
   * whose root the root contained (n-0173).
   */
  const code = loc.code?.path ?? dirname(loc.spec.path);
  let item = rowAbout(items, { id: String(entry.id), root: code, spec: loc.spec.path });
  if (!item) {
    /*
     ...
     */
    listed.add(doc.createNode({ id: entry.id, roots: [tilde(code)] }));
    item = listed.items.at(-1);
  }
