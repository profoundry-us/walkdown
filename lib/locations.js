...
/**
 * @param {{ id: string, root: string, spec?: string | null, homeDir?: string | null,
 *   inRepo?: boolean, home?: string | null }} entry
 */
export function rememberProject({ id, root, spec = null, homeDir = null, inRepo = false, home = null }) {
  ...
  const paths = homeDir
    ? homePaths(homeDir)
    : { spec, runs: join(spec, 'runs'), threads: join(spec, 'threads') };
  /*
  ...
  const listed = (projects.items ?? []).find(
    (it) => item(it, 'spec') && expand(item(it, 'spec'), root) === expand(paths.spec),
  );
   ...
   */
  const rel = (p) => (inRepo ? (p === root ? '.' : relative(root, p)) : tilde(p));
  /*
...
export function readUserConfig({ cwd = process.cwd() } = {}) {
    ...
    return {
      ...
      config: {
        ...
        ...(personal.config.projects
          ? { projects: personal.config.projects.map((p) => mine(p ?? {})) }
          : {}),
      },
  ...
  const about = (p) => {
    ...
    return roots.some((r) => expand(r) && within(expand(r), base));
  };
  const shadowed = [];
  const own = [];
...
export function rememberLocation(loc, kind, to) {
  ...
  let item = items.find((it) => {
    ...
    return String(it.get('spec') ?? '') && expand(String(it.get('spec'))) === loc.spec.path;
  });
  if (!item) {
    /*
