...
export function registryEntry(raw) {
  const homeDir = raw.home ? expand(String(raw.home)) : null;
  const project = raw.project ? expand(String(raw.project)) : null;
  const { home: _h, project: _p, registered, ephemeral, ...rest } = raw;
  const paths = homeDir ? homePaths(homeDir) : {};
  const at = registered?.at ?? null;
  return {
    id: raw.id,
    ...paths,
    ...rest, // machine-local overrides win over the layout: evidence:, targets:
...
function rememberInRegistry({ id, root, homeDir, moved = {}, extra = {}, by = 'import', ephemeral = null }) {
  ...
  try {
    ...
    const listed = rows.find(
      (r) => r.home && same(r.home, home) && (!root || !r.project || same(r.project, root)),
    );
    ...
    const mine = root
      ? rows.find((r) => String(r.id) === String(id) && r.project && same(r.project, root))
      : null;
    if (mine) {
     ...
     */
    const folded = root && !ephemeral ? foldFromConfig({ id, root, spec: join(home, HOME_LAYOUT.spec) }) : {};
...
function foldFromConfig({ id, root, spec }) {
  ...
  try {
    ...
    const row = rowAbout(rows.items, { id, root, spec });
    if (!row) return {};
    const out = {};
...
export function rememberBlueprint({
  ...
  try {
    ...
    if (inRepo) {
      ...
      /*
       * And never a spec that another `.walkdown` already answers for. A pack
       * carrying its own `.walkdown` is a project of its own; a committed entry
       * from the root reaching down into it is the boundary crossing
       * one-walkdown-answers exists to forbid, written by hand (q-0168).
       */
      const theirs = walkdownRoot(dirname(paths.spec));
      if (
        theirs &&
        !insidePersonalHome(dirname(paths.spec)) &&
        canon(theirs) !== canon(join(base, '.walkdown'))
      )
        throw new Error(
          `${paths.spec} lies under ${theirs}, which already answers for it — that .walkdown is where it is declared, and a throwaway copy is listed with --ephemeral`,
        );
    }
    /*
...
/*
 * Is this spec inside the personal home — the one directory whose config.yml
 * IS the file doing the declaring?
 *
 * The crossing guard asks `walkdownRoot()` which .walkdown answers for a
 * spec, and walkdownRoot skips the personal home by construction: it looks
 * for the nearest .walkdown AT OR ABOVE a directory and never returns the
 * home itself. So a blueprint living in the personal home was always
 * attributed to whatever .walkdown happened to sit above that home — and the
 * exemption the rule's own step describes ("the personal home, whose config
 * that file is") was unreachable. It only looked like it held because the
 * default ~/.walkdown has nothing above it and walkdownRoot returns null
 * first.
 *
 * Put WALKDOWN_HOME inside a checkout — which is exactly what this project's
 * own test runner does — and `init --commit none` created a project its own
 * checkout could not then see: init printed "+ listed … as other" and every
 * command after it said nothing claims this directory (n-0213). The personal
 * home wins for its own blueprints (Topher, 2026-09-06); the alternative was
 * refusing that layout at init, which is more honest and breaks the runner.
 */
function insidePersonalHome(dir) {
  const home = canon(walkdownHome());
  const at = canon(dir);
...
export function readUserConfig({ cwd = process.cwd() } = {}) {
  ...
  /*
   * The registry's rows read as personal rows (ADR 0003 step 1). The
   * `blueprints:` list in config.yml is still read this step, so a row a
   * person kept there keeps meaning what it meant; the two are told apart on
   * the report by which file each came from.
   */
  const registry = readRegistry();
  personal.config = {
    ...personal.config,
    blueprints: [...(personal.config.blueprints ?? []), ...registry.rows.map(registryEntry)],
  };
   ...
   * relative" - a `roots: ['  ']` expanded to wherever the command ran and
   * claimed it (n-0175).
   */
  const stray = (v) => {
  ...
  const anchored = (p) => {
    ...
    const out = { ...p };
    const roots = [p.roots ?? []].flat().filter((r) => r !== undefined && r !== null);
    const kept = roots.filter((r) => !stray(r));
    for (const r of roots) if (stray(r)) setAside(p.id ?? null, 'roots', r, stray(r));
    ...
    for (const k of ['spec', ...KINDS])
      if (stray(p[k])) {
        setAside(p.id ?? null, k, p[k], stray(p[k]));
        delete out[k];
      }
    return out;
  };
  /*
   * AND THE SAME BOUNDARY, IN THE PERSONAL FILE.
   *
   * A committed entry reaching under another `.walkdown` is set aside below,
   * because that directory answers for the spec. Written into the personal
   * file instead, the identical row was honoured everywhere: `blueprints` listed
   * it, `where --blueprint` resolved the other pack's spec, runs, threads and
   * evidence, the server offered it and a POST wrote a thread into the pack's
   * own ledger (n-0188).
   *
   * What settles it is not the harm alone but the contradiction: `where`
   * printed, in one block, that it resolved the personal row and refused the
   * byte-identical committed one. A tool that answers the same question two
   * ways in the same breath is worse than one that is merely strict.
   *
   * Two `.walkdown` directories are a row's own to declare from. The personal
   * home, because blueprints kept out of a repository live under it and this
   * file IS its config; and the one answering where you stand, because that is
   * what standing in a checkout means. Anything else is somebody else's board.
   */
  const wdOwn = repoPath ? canon(join(dirname(dirname(repoPath)), '.walkdown')) : null;
  const mineToDeclare = canon(walkdownHome());
  const refused = [];
  const crosses = (spec) => {
    if (!spec) return null;
    const at = dirname(expand(String(spec)));
    // A blueprint in the personal home is the personal home's to declare,
    // whatever sits above that home in the filesystem (n-0213).
    if (insidePersonalHome(at)) return null;
    const theirs = walkdownRoot(at);
    if (!theirs) return null;
    const t = canon(theirs);
    if (t === mineToDeclare || (wdOwn && t === wdOwn)) return null;
    /*
     * And only if that directory can actually answer. A `.walkdown` holding
     * homes but no `config.yml` declares nothing, so nothing there is claiming
     * the spec and this row is its only declaration - which is a monorepo pack
     * whose entries a person keeps personally, not a boundary crossing. The
     * harm this refusal is for needs somebody on the other side: in n-0188 the
     * pack's own config declared the blueprint, and two boards then listed,
     * served and wrote to one ledger.
     */
    return existsSync(join(theirs, 'config.yml')) ? theirs : null;
  };
  const personalBlueprints = (personal.config.blueprints ?? []).map(anchored).filter((p) => {
    /*
     * IMPORTED IS THE ONE WAY ACROSS (ADR 0001).
     *
     * Everything below refuses a personal row reaching under another
     * `.walkdown`, because in n-0188 two boards listed, served and wrote to
     * one ledger with nobody having asked. What made that harm possible was
     * INFERENCE: the row was a copy, the boundary was invisible, and the
     * second board arrived by accident.
     *
     * `walkdown import` is the opposite act. A person names a directory, is
     * shown what it declares, and says which of them to take; the row carries
     * `imported:` saying where it came from and when. The registry is the
     * machine's list of projects it has been told about, and a server offers
     * that list rather than whatever `.walkdown` it happens to stand in.
     *
     * So the boundary now forbids inference, not breadth. Every other refusal
     * here stands, including for a row somebody hand-writes without importing.
     */
    if (p?.imported) return true;
    const under = crosses(p?.spec);
    if (!under) return true;
    refused.push({ id: String(p.id ?? '?'), spec: String(p.spec), under, from: 'personal' });
    return false;
  });
  /*
   * `defaults:` in the personal file, by the same rule. A relative default
   * was expanded wherever the command ran, so one project answered with two
   * drafts directories depending on the directory the question came from
   * (n-0175). The `{id}` inside it is fine; the base is what is missing.
   */
  const personalDefaults = { ...(personal.config.defaults ?? {}) };
  for (const [k, v] of Object.entries(personalDefaults))
    if (stray(v)) {
      setAside(null, `defaults.${k}`, v, stray(v));
      delete personalDefaults[k];
    }
  const personalConfig = { ...personal.config, defaults: personalDefaults };
  if (!Object.keys(personalDefaults).length) delete personalConfig.defaults;
  if (!repoPath)
    return {
      path,
      ...
      config: {
        ...personalConfig,
        ...(personal.config.blueprints ? { blueprints: personalBlueprints.map((p) => mine(p)) } : {}),
      },
  ...
  /*
   * A COMMITTED ENTRY THAT REACHES UNDER ANOTHER `.walkdown` IS NOT READ.
   *
   * That directory answers for the spec; a root entry naming it is the
   * boundary crossing one-walkdown-answers forbids, written by hand. Lint
   * reports it - and until now every reader still served it, listed it and
   * wrote to it while it stood, so the report and the behaviour disagreed
   * (q-0168). It is set aside here, once, and named on the report; nothing
   * downstream ever sees it as a project.
   */
  const committed = (shared.blueprints ?? []).filter((p) => {
    if (!p?.id) return false;
    if (!p.spec) return true;
    const theirs = walkdownRoot(dirname(expand(String(p.spec), base)));
    if (!theirs || canon(theirs) === wdOwn) return true;
    refused.push({ id: String(p.id), spec: String(p.spec), under: theirs, from: 'repo' });
    return false;
  });
  const byId = new Map(
    committed.map((p) => [
   ...
   * from that file's own entries, BEFORE the personal merge below can
   * overwrite their roots. Read after it, a personal override narrowing
   * `roots:` made the repository's row deny an entry the committed file
   * plainly held (n-0167, n-0175).
   */
  const repoRooted = [...byId.values()].some((p) =>
    [p.roots ?? []].flat().some((r) => expand(r) && within(cwd, expand(r))),
  );
  /*
   * The repository's `defaults:` hang off the repository, like its entries.
   * A relative default resolved at cwd is issue #7 wearing a fourth costume.
   */
  const sharedDefaults = Object.fromEntries(
    Object.entries(shared.defaults ?? {}).map(([k, v]) => [
      k,
      typeof v === 'string' && v.trim() && !isAbsolute(v.trim()) && !v.trim().startsWith('~')
        ? join(base, v.trim())
        : v,
    ]),
  );
  ...
  const shadowed = [];
  const own = [];
  for (const p of personalBlueprints) {
    if (!p?.id) continue;
    ...
    if (!about(p, prev)) {
       ...
       * own root (blueprintFor picks the longest root), so it is carried as
       * its own project. It is SHADOWED only where the repository's row
       * answers instead of it - standing in the nested pack it is rooted
       * at, it is the one answering, and saying it was shadowed there was a
       * false sentence (n-0173). Either way it never merges into the
       * repository's.
       */
      const theirs = blueprintFor(cwd, { blueprints: [prev] });
      const its = blueprintFor(cwd, { blueprints: [p] });
      if (theirs && (!its || canon(its.root).length <= canon(theirs.root).length)) shadowed.push(p.id);
      own.push(mine(p));
      continue;
    }
    /*
     * `home:` is a NAME, read against the `.walkdown` whose listing allocated
     * it. On an override of a committed entry the personal file allocated
     * nothing: a copied `home: 0002-gamma` pointed at a directory nobody
     * made under ~/.walkdown, keyed by a number handed out in a different
     * listing, and gamma's ledger split in two (n-0174). The committed
     * entry's home is the only one that can mean anything for it.
     */
    const over = { ...p };
    if (over.home !== undefined && over.home !== null) {
      setAside(p.id, 'home', over.home, 'a home is named by the file whose listing allocated it — the committed entry’s stands');
      delete over.home;
    }
    /*
    ...
    byId.set(
      p.id,
      declaredBy(
        withMarks({ ...prev, ...over }, marks(Object.keys(over), 'personal', prev[DECLARED_IN])),
  ...
  const merged = {
    ...shared,
    ...personalConfig,
    defaults: { ...sharedDefaults, ...personalDefaults },
    blueprints: [...byId.values(), ...own],
  };
 ...
 */
function blueprintFor(cwd, config) {
  let best = null;
  for (const p of config.blueprints ?? []) {
    for (const root of [p.roots ?? []].flat()) {
      const r = expand(root);
      if (!r || !within(cwd, r)) continue;
      if (!best || canon(r).length > canon(best.root).length) best = { blueprint: p, root: r };
    }
  }
  return best;
}
 ...
 *       pick: { candidates: string[], picked: string | null, spec: string | null, why: string, agrees: boolean } } | null,
 *     repo: { path: string, exists: boolean, error: string | null, matched: boolean,
 *       refused: { id: string, spec: string, under: string, from: string }[] } | null,
 *     refused: { id: string, spec: string, under: string, from: string }[] },
...
export function resolveLocations({
   ...
   * answers for is written down, and `walkdown blueprint add` is how something
  ...
  const matched = want
    ? (() => {
         ...
         * `--blueprint app` from inside one of them means that one (n-0160).
         */
        /*
         * "The one rooted where you are" is the MOST SPECIFIC root that
         * contains cwd, exactly as it is without `--blueprint`: mono/app and
         * mono/app/packs/app both contain the pack, and the first same-id
         * row whose roots did was the root's, so `thread new --blueprint app`
         * from inside the pack filed into the root's ledger, one flag away
         * from the bare command that filed into the pack's (n-0173).
         */
        const named = (config.blueprints ?? []).filter((x) => x?.id === want);
        const p =
          blueprintFor(cwd, { blueprints: named })?.blueprint ??
          named.find((x) => declaringFiles(x) !== 'personal') ??
          named[0];
        return p ? { blueprint: p, root: null } : null;
      })()
    : at
      ? (() => {
          /*
          ...
          const p = (config.blueprints ?? []).find((x) => x?.spec && expand(x.spec) === want_);
          return p ? { blueprint: p, root: null } : null;
        })()
      : blueprintFor(cwd, config);
  const entry = matched?.blueprint ?? null;
   ...
   */
  let spec;
  if (entry?.spec) spec = { path: expand(entry.spec), why: configWhy(entry, 'spec') };

  const code = gitRoot(spec?.path ?? cwd) ?? gitRoot(cwd);
  const id = entry?.id
    ? slug(entry.id)
    : ((spec && declaredName(spec.path)) ?? slug(basename(code ?? cwd)));
  ...
  const homeFor = (() => {
    ...
    const root = declaredIn(entry, 'home') === 'repo' ? wdRoot : home;
    if (!root) return null;
    return {
      dir: join(root, 'blueprints', String(entry.home)),
      why:
        declaredIn(entry, 'home') === 'repo'
  ...
  const fromDefault = (v) => {
    ...
    return entry?.id ? expand(raw.replace('{id}', slug(entry.id))) : null;
  };
  ...
  if (!spec) {
    ...
    if (at)
      spec = {
        path: null,
        why: `nothing declares ${expand(at, cwd)} — every blueprint walkdown answers for is written down; \`walkdown init\` starts one, \`walkdown blueprint add <home>\` lists one`,
      };
    else if (!homeFor)
      spec = {
        path: null,
        why: want
          ? `no blueprint \`${want}\` — \`walkdown blueprints\` lists them`
          : 'nothing declares this directory — `walkdown init` starts a project, or `walkdown blueprint add` lists an existing one',
      };
    else {
  ...
  for (const kind of KINDS) {
     ...
     * There is no shape after this one. Records used to be looked for INSIDE
     * the spec - `blueprint/runs/`, the layout every project had before homes
     * - and an existing directory there outranked everything. That was the
     * legacy layout kept alive on the read path, and it is gone: every
     * blueprint walkdown answers for is declared, and its entry names every
     * record or the home they derive from. An entry naming neither is a hand
     * edit, and the honest answer is no path and the reason.
     */
    out[kind] = homeFor
      ? { path: builtIn(kind), why: homeFor.why }
      : {
          path: null,
          why: `the entry names no ${kind} and no home — walkdown writes both; list the project again with \`walkdown blueprint add\``,
        };
  }
  ...
  const entryRoot = entry?.roots ? [entry.roots].flat().map((r) => expand(r))[0] : null;
  ...
  const declaredBy = entry ? declaringFiles(entry) : null;
  /*
  ...
  return {
    ...
    config: {
      ...
      ignored: cfgIgnored,
      // The registry (ADR 0003): what this machine knows about, and whether
      // the row that answered came from it.
      registry: cfgRegistry && {
        ...cfgRegistry,
        matched: Boolean(entry?.registered),
        registeredBy: entry?.registered
          ? `${entry.registered.by ?? 'walkdown'} on ${String(entry.registered.at ?? '').slice(0, 10)}`
          : null,
        /*
         * The registry's own answer, beside the walk's (ADR 0003 step 2).
         * `agrees` is by spec, never by id: an imported row and the manifest
         * it came from name one directory under two provenances.
         */
        pick: (() => {
          const pick = registryPick(cwd, readRegistry().rows, want);
          const pickedSpec = pick.picked ? join(expand(String(pick.picked.home)), HOME_LAYOUT.spec) : null;
          return {
            candidates: pick.candidates,
            picked: pick.picked ? String(pick.picked.id) : null,
            spec: pickedSpec,
            why: pick.why,
            agrees:
              pickedSpec && spec?.path
                ? canon(pickedSpec) === canon(spec.path)
                : pickedSpec || spec?.path
                  ? false
                  : true,
          };
      ...
      repo: cfgRepo && {
        ...cfgRepo,
        /*
         * Whether the committed file names THIS project. Asked by id, the
         * project is the id, and the row is about that - not about the
         * directory the command ran in, which is what `repoRooted` says
         * ("names this project" over "no project ghost", n-0175).
         */
        matched: declaredBy === 'repo' || declaredBy === 'both' || (repoRooted && !want),
        // Committed entries set aside because another `.walkdown` answers
        // for their spec (q-0168) - named, so the report can say why an
        // entry the file plainly holds is not a project here.
        refused: (cfgRefused ?? []).filter((r) => r.from === 'repo'),
      },
      // Both files' refusals, so a reader that has no committed config still
      // sees the personal ones (n-0188).
      refused: cfgRefused ?? [],
    },
    code: codePath
      ? {
          path: codePath,
          why: entryRoot
            ? configWhy(entry, 'roots', ', which says where the code is')
            : specRepo
...
export function canRemember(loc) {
  if (!loc.blueprint)
    throw new Error('nothing declares this directory — there is no entry to write to');
  /*
...
export function rememberLocation(loc, kind, to) {
  ...
  try {
    ...
    if (!row) {
      const folded = foldFromConfig({ id: String(entry.id), root: code, spec: loc.spec.path });
      row = {
        id: entry.id,
