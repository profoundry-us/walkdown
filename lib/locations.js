...
export function foldLegacyHomes() {
  ...
  for (const h of homes) {
    ...
    for (const kind of ['evidence', 'drafts']) {
      ...
      const named = item.get(kind);
      if (named) {
        if (canon(expand(String(named))) !== canon(at)) elsewhere.push(kind);
        continue;
      }
      item.set(kind, at);
      touched = true;
    ...
    if (!touched) {
      /*
       * "Already named" only when the config names THIS directory. An entry
       * carrying its own evidence key elsewhere - init's home, say - is not
       * naming the old home, and saying it was told a person the fold had
       * done something it had not (n-0173). Which of the two is the real
       * ledger is theirs to say.
       */
      found.push(
        elsewhere.length
          ? {
              path: h.path,
              act: 'left',
              why: `\`${entry.id}\` already names ${elsewhere.join(' and ')} elsewhere — say which is the real one`,
            }
          : { path: h.path, act: 'already', why: 'already named by the config' },
      );
      continue;
    }
    written++;
