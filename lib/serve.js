...
import { ROLES, normalizeRoles, writeRunRecord } from './run-record.js';
...
function configuredRoles(cwd) {
  ...
  const roles = raw.split(/[,\s]+/).map((r) => r.trim()).filter((r) => ROLES.includes(r));
  return {
...
export function createWalkdownServer(blueprintDir) {
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      /*
       * The screenshots a run attached. The ledger records them as paths
       * relative to the blueprint folder ("runs/evidence/<run>/<shot>.png"),
       * so that is exactly what this takes - and nothing outside that folder
       * is servable, because a count of pictures is not evidence until they
       * can be looked at, and everything else here is nobody's business.
       */
      if (req.method === 'GET' && url.pathname.startsWith('/evidence/')) {
        const rel = normalize(decodeURIComponent(url.pathname.slice('/evidence/'.length)))
          .replace(/^(\.\.[/\\])+/, '');
        if (!rel.startsWith('runs/evidence/')) {
          res.writeHead(404);
          return res.end('not found');
        }
        return serveFile(res, join(blueprint.dir, rel));
      }
