...
export function createWalkdownServer(blueprintDir, { cwd = process.cwd() } = {}) {
  ...
  return createServer(async (req, res) => {
    ...
    if (requested) {
       ...
       */
      const byKey = projects.find((p) => p.key === requested);
      const byId = projects.filter((p) => p.id === requested);
     ...
     *
     * The place whose `.walkdown` answered, which is the same place `projects`
    ...
    blueprint.root = servedFrom.startsWith(homedir())
      ...
      : servedFrom;
    blueprint.listed = projects.map((p) => ({
      id: p.id,
      /*
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/whose') {
        ...
        const matches = claimantsFor(index, asked)
          ...
          .filter((m) => projects.some((p) => p.key === m.key))
          .map((m) => ({
            ...m,
            id: projects.find((p) => p.key === m.key)?.id ?? m.id,
          }));
