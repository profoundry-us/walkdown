...
function frontDoor(blueprint) {
  ...
  const screen = screens.find((s) => s.id === wanted)
    ?? screens.find((s) => s.app?.path || s.prototype);
...
export function createWalkdownServer(blueprintDir) {
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      /*
       * One tag for the script-tag delivery. The panel and the embed stay two
       * files because the EXTENSION loads them into two different documents -
       * the embed into every page it visits, the panel only into walkdown's own
       * - and a single file could not tell those apart without drawing a panel
       * on every site you browse. But an app that carries walkdown itself puts
       * both in one document, and asking it for two tags in a fixed order was
       * a way to get the order wrong (n-0085).
       */
      if (req.method === 'GET' && url.pathname === '/walkdown.js') {
        const panel = readFileSync(join(VIEWER_DIR, 'panel.js'), 'utf8');
        const embed = readFileSync(join(VIEWER_DIR, 'embed.js'), 'utf8').replace(
          '__ANCHOR_ATTR__',
