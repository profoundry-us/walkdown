...
import { blueprintsForUrl } from './claims.js';
    ...
    try {
      ...
      if (handlers[key]) {
        ...
        if (key === 'POST /api/threads' && !requested && body?.url) {
          const loaded = projects.map((p) => ({
            id: p.key,
            dir: p.dir,
            blueprint: loadBlueprint(p.dir, { cwd }),
          }));
          // Exactly one, or nothing. Several claimants is a question for the
          // person at the panel, and a door is not where it gets answered
          // (ADR 0001) - this whole re-routing goes when the panel always
          // names the blueprint it has open.
          const hits = blueprintsForUrl(loaded, body.url);
          const whose = hits.length === 1 ? hits[0] : null;
      ...
      if (req.method === 'GET' && url.pathname === '/') {
        const html = readFileSync(join(VIEWER_DIR, 'review.html'), 'utf8')
          ...
          .replace('__BLUEPRINT__', blueprint.projects.find((p) => p.current)?.key ?? '');
