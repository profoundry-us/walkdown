...
import { resolveLocations, walkdownRoot } from './locations.js';
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/whose') {
        ...
        // person reads the id.
        const loaded = projects.map((p) => ({ id: p.key, blueprint: loadBlueprint(p.dir, { cwd }) }));
        const matches = blueprintsForUrl(loaded, asked).map((hit) => ({
          ...hit,
          key: hit.id,
          id: projects.find((p) => p.key === hit.id)?.id ?? hit.id,
        }));
