...
import { resolveLocations, tilde, walkdownRoot } from './locations.js';
 ...
 */
/**
 * The project a blueprint lives in: the directory holding the `.walkdown`
 * that declares it. Falls back to the home's parent, which is what a
 * blueprint kept outside any repository has.
 */
function projectOf(specDir) {
  const wd = walkdownRoot(specDir);
  const root = wd ? dirname(wd) : resolve(specDir, '..', '..');
...
export function createWalkdownServer(blueprintDir, { cwd = process.cwd() } = {}) {
  ...
  return createServer(async (req, res) => {
     ...
     */
    const answering = walkdownRoot(cwd);
    const servedFrom = answering ? dirname(answering) : baseRoot;
