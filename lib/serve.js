...
export function createWalkdownServer(blueprintDir) {
  ...
  const evidenceRoot = (() => {
    try {
      return resolveLocations({ dir: blueprintDir }).evidence.path;
    } catch {
  ...
  const handlers = {
    ...
    'POST /api/draft': (blueprint, _req, _url, body) => {
      ...
      const rulesById = new Map(
        collectRules(blueprint.features).map(({ rule }) => [rule?.id, rule]),
      );
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && threadAction) {
        ...
        const thread =
          action === 'replies'
            ? replyToThread(blueprint, id, {
                author: body.author || userInfo().username,
                body: body.body,
              })
            : transitionThread(blueprint, id, {
                status: body.status,
                actor: body.actor,
                reason: body.reason,
              });
...
/** @param {string} blueprintDir @param {{ port?: number }} [opts] */
export function startServe(blueprintDir, { port } = {}) {
  ...
  return new Promise((resolvePromise) => {
    server.listen(chosenPort, '127.0.0.1', () =>
      resolvePromise({ server, port: /** @type {import('node:net').AddressInfo} */ (server.address()).port }),
    );
  });
}
