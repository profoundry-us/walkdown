...
export function scaffold(root, { force = false, specDir = null, skills = null, commit = 'none' } = {}) {
  root = resolve(root);
  /*
   * Where the spec goes - always inside the project's own `.walkdown` now, so
   * the question is no longer WHERE but whether git is watching. Everything
   * that used to key off "is the spec in the tree" keys off that instead:
   * under the new layout the spec is in the tree either way, and reading the
   * path would answer "committed" for a directory that is ignored
   * (docs/08-locations.md, n-0154).
   */
