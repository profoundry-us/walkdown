...
export function placePointer(file, block) {
  ...
  /*
   * The block ends at the end of its marker's own LINE, not one character
   * after the marker. `+ POINTER_END.length + 1` assumed exactly one newline
   * follows and swallowed whatever was really there: a trailing space left a
   * space-only line outside the block, a CRLF file lost its \r and gained a
   * blank line nobody wrote, and a person's own comment written after the
   * marker lost its leading `<` and became broken markup (n-0187). The
   * statement is absolute about owning nothing else in the file, and eating a
   * character of somebody's text is the plainest way to break that.
   */
  ...
  writeFileSync(file, text.slice(0, from) + block + text.slice(after));
...
/**
 * Take the skills back out of a project's `.claude/skills/` when the spec
 * leaves the repository. Skills follow the spec both ways: the copies that
 * arrived with `--commit spec` would otherwise stay staged in a repository
 * the command has just said gets nothing (n-0166). Only walkdown's own,
 * unedited copies go - an edited one was meant, and is kept and named.
 * Directories the removal empties go with it, so a project is left as it
 * was found.
 */
export function removeSkills(from) {
  const results = skillFiles().flatMap(({ name, content }) => {
    const abs = join(from, name, 'SKILL.md');
    if (!existsSync(abs)) return [];
    if (readFileSync(abs, 'utf8') !== content) return [{ path: abs, action: 'kept-edited' }];
    rmSync(abs);
    if (!readdirSync(dirname(abs)).length) rmSync(dirname(abs), { recursive: true });
    return [{ path: abs, action: 'removed' }];
  });
  for (const dir of [from, dirname(from)])
 ...
 *
 * @param {string} root
 * @param {{ force?: boolean, specDir: string, skills?: string | null, commit?: string }} opts
 */
export function scaffold(root, { force = false, specDir, skills = null, commit = 'none' }) {
  root = resolve(root);
  /*
