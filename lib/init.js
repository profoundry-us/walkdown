...
export function scaffold(root) {
  ...
  // Procedural skills for Claude Code sessions in this repo. Skip-if-exists:
  // a team's customized copy is never clobbered.
  for (const file of readdirSync(SKILLS_DIR).filter((f) => f.endsWith('.md')).sort()) {
    const name = file.replace(/\.md$/, '');
    const target = join(root, '.claude', 'skills', name, 'SKILL.md');
    if (existsSync(target)) {
      created.push(`.claude/skills/${name}/SKILL.md (kept existing)`);
      continue;
    }
    mkdirSync(join(root, '.claude', 'skills', name), { recursive: true });
    writeFileSync(target, readFileSync(join(SKILLS_DIR, file), 'utf8'));
    created.push(`.claude/skills/${name}/SKILL.md`);
  }
  return created;
}
