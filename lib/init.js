 ...
 */
const SHIPPED = ['walkdown-setup', 'walkdown-formulate', 'walkdown-judge', 'walkdown-incorporate', 'walkdown-backlog'];
...
export function skillFiles() {
  return readdirSync(SKILLS_DIR)
    ...
    .sort()
    .map((f) => ({ name: f.replace(/\.md$/, ''), content: readFileSync(join(SKILLS_DIR, f), 'utf8') }));
}
...
export function installSkills(into, { force = false } = {}) {
  return skillFiles().map(({ name, content }) => {
    ...
    if (!existsSync(abs)) { writeFileSync(abs, content); return { path: abs, action: 'created' }; }
    if (readFileSync(abs, 'utf8') === content) return { path: abs, action: 'up-to-date' };
    if (force) { writeFileSync(abs, content); return { path: abs, action: 'updated' }; }
...
export function scaffold(root, { force = false, specDir = null, skills = null } = {}) {
  ...
  for (const r of installSkills(skillsInto, { force }))
    results.push({ ...r, path: r.path.startsWith(root + '/') ? r.path.slice(root.length + 1) : r.path });
  results.push({ path: skillsInto, action: skillsInto.startsWith(root + '/') ? 'skills-in-repo' : 'skills-personal' });
