...
| **Note** | A remark pinned to a rule/screen/element, carrying a **reason** that says what it is: a person's `feedback`, a judge's `finding`, an agent's `observation` (the agent may settle it), a `request` (a rule not yet written), or a `decision` (filed `recorded`, in no queue). On a rule the walk can reach, every note but a decision is part of the rule's **conversation** and closes on the person's next signed pass (ADR 0006); off the walk, a person verifies or waives it from the thread's screen (ADR 0005). | Punch-list item |
| **As-built** | A drawing of what was actually built, kept by engineering under `as-built/` for a build walkdown cannot frame (its own panel, a native app) and named as a screen's app path. Redlined where it left the design; never the design, never evidence. ADR 0007. | As-built drawing (construction) |
| **Conversation** | Everything ever said on one rule, drawn as one stream under it: each thread's opening message tagged with the thread, its kind and status. A rule's verdict is what the conversation waits on - a pass ends it, a fail continues it. ADR 0006. | Discussion thread |
