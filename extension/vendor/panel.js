...
(() => {
  ...
  const SERVER = cfg.server ?? new URL(script?.src ?? 'http://localhost:4700').origin;
  const BP = cfg.bp ?? script?.dataset.bp ?? '';
  ...
  bar.style.cssText = `position:absolute; top:0; left:0; right:0; height:${TOP}px;
    pointer-events:auto; transition:transform .2s ease;`;
  bar.className = 'flex items-center gap-2 border-b border-base-300 bg-base-100 px-3 text-base-content';
  const side = document.createElement('aside');
  side.dataset.theme = 'blueprint';
  side.style.cssText = `position:absolute; top:${TOP}px; right:0; bottom:0; width:${W}px;
    pointer-events:auto; transition:transform .22s ease;`;
  side.className = 'flex flex-col border-l border-base-300 bg-base-100 text-base-content';
  ...
  function paintDesk(on) {
    ...
    root.style.margin = `${TOP + GAP}px ${W + GAP}px ${GAP}px ${GAP}px`;
    ...
    page.style.minHeight = `calc(100vh - ${TOP + GAP * 2}px)`;
  ...
  function setDocked(on) {
    ...
    side.style.transform = on ? 'none' : 'translateX(100%)';
    ...
    document.documentElement.style.setProperty('--walkdown-dock', on ? `${W + GAP}px` : '0px');
  ...
  function renderBar() {
    ...
    const showing = ghost && ghostOpacity > 0.99 ? 'prototype' : 'app';
    ...
    bar.innerHTML = `
      ...
      ${projects.length > 1
        ? `<select class="select select-xs ml-1 w-52" id="wdp-project" title="Which project's blueprint">
            ${projects.map((p) => `<option value="${esc(p.id)}" ${p.current ? 'selected' : ''}>${esc(p.name)}</option>`).join('')}
          </select>`
        : `<span class="text-[11.5px] opacity-50">${esc(data.project)}</span>`}
      <span class="flex-1"></span>
      <span class="join" id="wdp-surface" title="${canGhost ? 'Compare the design against the page'
        : 'No design on file for this screen'}">
        <button class="btn btn-xs join-item${showing === 'prototype' ? ' btn-active' : ''}"
          data-surface="prototype" ${canGhost ? '' : 'disabled'}>Prototype</button>
        <button class="btn btn-xs join-item${showing === 'app' ? ' btn-active' : ''}" data-surface="app">App</button>
      </span>
      <button class="btn btn-xs${pinning ? ' btn-warning' : ''}" id="wdp-pin"
        title="Click anything on the page to attach a note">📌 Pin mode</button>
      <button class="btn btn-xs btn-ghost" id="wdp-undock" title="Put walkdown away">×</button>`;

    const proj = bar.querySelector('#wdp-project');
    if (proj) proj.onchange = () => { location.search = '?bp=' + encodeURIComponent(proj.value); };
    ...
    bar.querySelectorAll('[data-surface]').forEach((b) => {
      b.onclick = () => {
        // Full strength is "show me the design"; off is "show me what shipped".
        // The footer's slider still owns everything in between.
        if (b.dataset.surface === 'app') return setGhost(false);
        ghostOpacity = 1;
        if (ghost) { ghost.style.opacity = 1; render(); } else setGhost(true);
      };
    });
  }
  ...
  function setGhost(on) {
    ...
    ghost.style.cssText = `position:fixed; top:${TOP + GAP}px; left:${GAP}px; bottom:${GAP}px;
      right:${W + GAP}px; z-index:2147482000; border-radius:10px; overflow:hidden;
  ...
  window.walkdownEmbed?.watchPinMode(() => { if (data) renderBar(); });

  load().catch(() => {
    side.innerHTML = '<p class="p-3.5 text-[12.5px] opacity-40">walkdown server unreachable — run <code>walkdown serve</code>.</p>';
  });
})();
