...
(() => {
  ...
  function render() {
    ...
    renderBar();
    const TAB_ICON = { blueprints: 'blueprint', rules: 'checks', screens: 'frame-corners' };
    const tab = (id, label) =>
      `<button role="tab" class="tab gap-1${listTab === id ? ' tab-active' : ''}" data-tab="${id}">
        ${icon(TAB_ICON[id], 'size-3.5')}${label}</button>`;
    side.innerHTML = `
