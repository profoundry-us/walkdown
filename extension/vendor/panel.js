(function () {
  ...
  function detailPane() {
    ...
    const steps = r.steps
      ? Object.entries(r.steps).map(
          ([ph, items]) =>
            b`<span class="${LBL} pt-1">${ph}</span><ul class="list-disc pl-4">${items.map(
  ...
  function renderBar() {
    ...
    ${
      STALE_COPY()
        ? b`<span class="badge badge-sm badge-error badge-dash min-w-0 gap-1 truncate font-semibold"
