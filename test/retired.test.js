...
function fixture() {
  ...
  writeFileSync(
    ...
    [
      ...
      '        verify: [checks]',
      '        steps:',
      `          statement_hash: "${formatHash(live)}"`,
      '      - id: demo.main.gone',
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      target: 'local',
      results: [
        { rule: 'demo.main.live', status: 'pass' },
        { rule: 'demo.main.gone', status: 'pass' },
      ],
    }),
  );
  return bp;
...
test('a retired rule leaves the report but keeps its id resolvable', () => {
  ...
  assert.deepEqual(
    findings.filter((f) => f.subject === 'demo.main.gone'),
    [],
  );
});
...
test('a retired screen leaves the surfaces but keeps the threads anchored to it valid', () => {
  ...
  assert.deepEqual(
    findings.filter((f) => /unknown screen/.test(f.message)),
    [],
  );
  // And a retired screen with no design is not a missing design request.
  assert.deepEqual(
    findings.filter((f) => f.subject === 'gone'),
    [],
  );
