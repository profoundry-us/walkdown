...
const post = (body) => fetch(`${base}/api/walkdowns`, {
  method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body),
});
const recordFor = (runId) => JSON.parse(readFileSync(
  join(bp, 'runs', readdirSync(join(bp, 'runs')).find((f) => f.includes(runId))), 'utf8'));
...
test('a run carries the roles its signer was acting in @rule:status.acceptance.roles-recorded-on-the-run',
  () => {
    const { record } = writeRunRecord({
      blueprintDir: join(root, 'direct'), target: 'local', actor: 'topher',
      kind: 'walkdown', roles: ['eng', 'product'],
    ...
    for (const roles of [undefined, null, [], ['', '  ']]) {
      const { record: r } = writeRunRecord({
        blueprintDir: join(root, 'direct'), target: 'local', actor: 'topher',
        kind: 'walkdown', roles, results: [{ rule: 'demo.main.thing', status: 'pass' }],
      });
...
test('the recording endpoint accepts roles and persists them @rule:status.acceptance.roles-recorded-on-the-run',
  async () => {
    const ok = await (await post({
      actor: 'topher', roles: ['eng', 'product'],
      results: [{ rule: 'demo.main.thing', status: 'pass' }],
    })).json();
    ...
    const empty = await (await post({
      actor: 'topher', roles: [], results: [{ rule: 'demo.main.thing', status: 'pass' }],
    })).json();
    ...
    const legacy = await (await post({
      actor: 'topher', results: [{ rule: 'demo.main.thing', status: 'pass' }],
    })).json();
    ...
    const bad = await post({
      actor: 'topher', roles: ['eng', 'marketing'],
      results: [{ rule: 'demo.main.thing', status: 'pass' }],
    });
...
test('the identity the server derives carries roles, and the vocabulary to change them @rule:status.acceptance.roles-recorded-on-the-run',
  async () => {
