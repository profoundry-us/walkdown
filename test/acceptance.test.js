  ...
  return {
    ...
    features: [{
      ...
      data: {
        ...
        stories: [{
          id: 'demo.main',
          rules: [{ id: 'demo.main.thing', statement: STATEMENT, verify, ...(signoff && { signoff }) }],
        }],
      },
    }],
    threads: [],
...
const checksRun = (created, status) => ({
  created, kind: 'checks', target: 'local', actor: 'agent', run_id: created,
  results: [{ rule: 'demo.main.thing', status, statement_hash: HASH }],
});
const agentRun = (created, status) => ({
  created, kind: 'walkdown', target: 'local', actor: 'agent', run_id: created,
  results: [{ rule: 'demo.main.thing', status, statement_hash: HASH }],
});
...
test('acceptance is derived per role from the roles the run was recorded under @rule:status.acceptance.role-comes-from-the-run',
  () => {
    const { rows } = deriveStatus(blueprint({
      signoff: ['eng', 'product'],
    ...
    const both = deriveStatus(blueprint({
      signoff: ['eng', 'product'],
    ...
    const two = deriveStatus(blueprint({
      signoff: ['eng', 'product'],
      runs: [
  ...
  () => {
    for (const status of ['refining', 'fail']) {
      const { rows, attention } = deriveStatus(blueprint({
        signoff: ['eng', 'product'],
        runs: [checksRun('2026-01-01', 'pass'), agentRun('2026-01-02', 'pass'),
      ...
      assert.equal(attention.some((i) => i.action === 'judge' && i.role === 'product'), false, status);
      // Engineering has still not signed, and is still asked to.
      assert.equal(attention.some((i) => i.action === 'judge' && i.role === 'eng'), true, status);
    }
  });
...
test('a verdict needs every tier AND every role @rule:status.acceptance.verdict-needs-every-role', () => {
  ...
  const half = deriveStatus(blueprint({
    signoff: ['eng', 'product'], runs: [...runs, signed('2026-01-03', 'topher', 'pass', ['eng'])],
  }));
  ...
  const whole = deriveStatus(blueprint({
    signoff: ['eng', 'product'],
  ...
  const broken = deriveStatus(blueprint({
    signoff: ['eng', 'product'],
    runs: [checksRun('2026-01-01', 'fail'), agentRun('2026-01-02', 'pass'),
...
test('the queue names the role a rule waits on @rule:status.attention.names-the-role', () => {
  const { attention } = deriveStatus(blueprint({
      ...
      data: {
        ...
        stories: [{
          ...
          rules: [{
            ...
            statement: STATEMENT,
            unverifiable: {
              checks: 'The control is the browser toolbar - a test that drives a page cannot click it.',
              agent: 'The same button, for the same reason - no tool an agent drives reaches browser chrome.',
            },
          }],
        }],
      },
